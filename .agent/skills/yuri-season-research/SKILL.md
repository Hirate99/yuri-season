...
4. Treat search results as leads. Open the original page and verify identity, entity match, date, authorship, and original-post status. Never turn a snippet into an observation. When redistribution is allowed, preserve the readable original-language body as `publicText`; a faithful Chinese translation may be added separately as `publicTranslation`. Keep `excerpt` as an internal paraphrase. A translation must never replace the original, and neither `excerpt` nor the editorial Chinese summary may be passed off as a translation.
...
- When an eligible 2.5D member post visibly contains an original image or official preview, create a linked media record so it appears in the work's image section as well as the feed. If a candidate title or summary says that an image, illustration, photo, visual, or video was posted, missing media is a preflight failure unless the media is unavailable or policy requires `link_only`. Keep `media.originalUrl` on the original post. For a verified official, creator, cast, or project-member account, the site may cache the exact public image bytes under the scoped internal media key when neither the post, profile, nor linked source page explicitly prohibits reposting or embedding. Preserve creator attribution, the upstream image URL and hash in internal evidence, plus the correction/takedown path. Keep the public `rightsNote` short, such as `图片来自原帖`; never expose bucket names, object keys, byte-cache wording, or other storage implementation details. This default does not extend to community fanwork, which remains `link_only` without affirmative reuse permission.
...
- When the original page exposes an actual image or official preview, add a linked `media` record as well as the feed candidate so the work page can show it in the art/fanwork gallery. Preserve the original page as `media.originalUrl`. When caching is permitted and the image is verified safe, store the exact fetched bytes using the existing internal media pipeline and set `media.previewUrl` with `remote_preview`; retain creator attribution, the upstream image URL and hash in evidence metadata, correction notices, safety, and spoilers. Public copy should say only `图片来自原帖` or `官方图片`; storage details remain internal. Never upload a placeholder, `NOW PRINTING` image, search thumbnail, an image whose original page cannot be verified, or any image whose source explicitly prohibits redistribution, reposting, or embedding; keep those entries `link_only`. If no actual image is available, omit `media` and do not title the item as though viewers can see the artwork.
