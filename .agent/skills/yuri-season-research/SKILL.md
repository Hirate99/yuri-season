...
- Treat stored datetimes as instants. Publicly returned timestamps must include `Z` or an explicit offset, and `publishedAt`, `capturedAt`, and correction times must all render in the same viewer timezone. When a source provides a calendar date but no clock time, set `publishedAt` to 12:00:00 in the source publisher's local IANA timezone on that date, resolve the offset that applies on that date, and store the resulting ISO datetime with an explicit offset (for example, a Japanese official source dated 2026-08-14 becomes `2026-08-14T12:00:00+09:00`). Never substitute UTC, the viewer or runtime timezone, or midnight for a date-only source. `capturedAt` must remain the actual capture instant. A zone-less SQLite `CURRENT_TIMESTAMP` value must be normalized before it reaches the client.
