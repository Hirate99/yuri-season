...
- Do not turn every routine stream, generic store campaign, ordinary ticket reminder, or incidental promotional appearance into a calendar event. Keep it as normal news when the date has little value apart from the announcement; include it when limited attendance, a distinctive program, meaningful cast/creator participation, or direct work relevance makes attendance or viewing time useful to readers.
...
- Use `eventType: event` for concerts, Lives, conventions, exhibitions, voice-actor meet-and-greets, stage greetings, Talk Events, signings, in-person stages, and hybrid events. Use `stream` for online-only scheduled programs or concerts. Keep ordinary broadcasts and product releases in their existing lanes.
