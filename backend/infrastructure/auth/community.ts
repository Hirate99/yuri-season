  ...
  return betterAuth({
    ...
    databaseHooks: {
      user: {
        create: {
          before: async (user) => ({
            data: { ...user, image: null, name: user.name.trim().slice(0, 32) || "新来的同好" },
          }),
        },
        update: {
          before: async (user) => {
            if (user.image !== undefined)
              throw new APIError("FORBIDDEN", { message: "头像上传暂未开放。" });
            ...
            return {
              data: { ...user, ...(user.name !== undefined ? { name: user.name.trim() } : {}) },
            };
          },
        },
      },
      ...
      emailOTP({
        ...
        async sendVerificationOTP({ email, otp }) {
          ...
          await env.EMAIL.send({
            from: { email: env.AUTH_EMAIL_FROM, name: "YuriSeason" },
            to: email,
            subject: `${otp} · YuriSeason 登录验证码`,
            text: `你的 YuriSeason 验证码是 ${otp}，5 分钟内有效。\n\n请勿向他人透露验证码。如果不是你本人操作，请忽略这封邮件。`,
          }).catch(() => {
            emailSendFailed = true;
            console.error(JSON.stringify({ event: "auth_email_send_failed" }));
          });
        },
      }),
    ],
