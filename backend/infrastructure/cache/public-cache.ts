...
async function redisClient(env: RedisEnvironment): Promise<PublicCacheClient | null> {
...
export async function readThroughPublicCache<T>(
  ...
  const client = options.client ?? await redisClient(env);
...
export async function invalidatePublicCache(
  env: RedisEnvironment,
  providedClient?: PublicCacheClient | null,
): Promise<void> {
  const client = providedClient ?? await redisClient(env);
  if (!client) return;
  try {
