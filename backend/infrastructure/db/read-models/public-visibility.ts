...
import { discussionsTable, feedCandidatesTable, feedItemsTable, mediaItemsTable } from "../schema";
  ...
  return or(
    ...
    exists(
      orm
        ...
        .from(feedItemsTable)
        .where(
          and(eq(feedItemsTable.mediaId, mediaItemsTable.id), isNull(feedItemsTable.withdrawnAt)),
        ),
    ),
  );
}
