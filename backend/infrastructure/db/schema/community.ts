...
export const communityThreads = sqliteTable(
  ...
  (t) => [
    index("community_threads_anime").on(t.animeId, t.hidden, t.pinned, t.lastActivityAt, t.id),
    index("community_threads_author").on(t.authorId, t.createdAt),
  ],
);
...
export const communityPosts = sqliteTable(
  ...
  (t) => [
    check(
      "community_posts_shape",
      sql`(${t.parentPostId} is null and ${t.floor} is not null and ${t.floor} > 0) or (${t.parentPostId} is not null and ${t.floor} is null)`,
    ),
    uniqueIndex("community_posts_floor").on(t.threadId, t.floor),
    index("community_posts_comments").on(t.parentPostId, t.createdAt, t.id),
    index("community_posts_author").on(t.authorId, t.createdAt),
  ],
);
...
export const communityLikes = sqliteTable(
  ...
  (t) => [
    ...
    index("community_likes_user").on(t.userId),
  ],
);
...
export const communityReports = sqliteTable(
  ...
  (t) => [
    uniqueIndex("community_reports_once").on(t.postId, t.reporterId),
    index("community_reports_queue").on(t.resolved, t.createdAt),
  ],
);
