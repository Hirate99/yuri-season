...
async function seasonStart(db: D1Database, animeId: string): Promise<string> {
  const row = await database(db).select({ startsOn: seasonsTable.startsOn }).from(animeTable)
...
export async function birthdayEventQueries(
  ...
  const remove = orm.delete(eventsTable).where(and(
  ...
  const startsAt = birthdayOccurrence(await seasonStart(db, animeId), value.birthdayMonth, value.birthdayDay);
