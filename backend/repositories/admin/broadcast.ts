...
export async function updateBroadcast(db: D1Database, animeId: string, id: string, value: BroadcastWrite, audit: ResourceChangeAudit): Promise<void> {
...
export async function deleteBroadcast(db: D1Database, animeId: string, id: string, audit: ResourceChangeAudit): Promise<D1Result> {
  const before = await resourceAuditSnapshot(db, animeId, "broadcast", id);
  const orm = database(db);
