...
export async function updateCast(db: D1Database, animeId: string, id: string, value: CastWrite, audit: ResourceChangeAudit): Promise<void> {
  const orm = database(db);
  const before = await orm.select().from(castCreditsTable)
  ...
  const queries: BatchItem<"sqlite">[] = [
    ...
    orm.update(charactersTable).set({
      ...
      birthdayVerified: value.birthdayVerified,
      sortOrder: value.sortOrder,
    }).where(and(eq(charactersTable.id, before.cast_credits.characterId), eq(charactersTable.animeId, animeId))),
...
export async function deleteCast(db: D1Database, animeId: string, id: string, audit: ResourceChangeAudit): Promise<D1Result> {
  const orm = database(db);
  const before = await orm.select().from(castCreditsTable)
    .innerJoin(charactersTable, eq(charactersTable.id, castCreditsTable.characterId))
    .where(and(eq(castCreditsTable.id, id), eq(castCreditsTable.animeId, animeId)))
