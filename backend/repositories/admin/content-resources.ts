...
export async function readAdminContentResources(
  db: D1Database,
  animeId: string,
): Promise<{
  events: AdminEvent[];
  ...
  const trackUsage = orm.$with("track_usage").as(
    orm
      .select({
  ...
  const [events, media, discussionRows, themeSongs, discussionLinks] = await orm.batch([
    orm
      ...
      .orderBy(
        sql`COALESCE(${eventsTable.startsAt}, '9999-12-31')`,
        eventsTable.title,
        eventsTable.id,
      ),
    orm
    ...
    orm
      ...
      .where(
        inArray(
          discussionAnimeTable.discussionId,
          orm
            .select({ id: discussionAnimeTable.discussionId })
            .from(discussionAnimeTable)
            .where(eq(discussionAnimeTable.animeId, animeId)),
        ),
      ),
