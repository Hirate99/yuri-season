...
export async function readAdminCoverage(db: D1Database): Promise<AdminAnimeCoverage[]> {
  ...
  const [
    ...
    }).from(animeTable).orderBy(desc(animeTable.seasonId), animeTable.titleZh, animeTable.id),
    orm.select({ animeId: broadcastSlotsTable.animeId, count: count() })
      .from(broadcastSlotsTable).groupBy(broadcastSlotsTable.animeId),
    orm.select({ animeId: workCreditsTable.animeId, count: count() })
      .from(workCreditsTable).groupBy(workCreditsTable.animeId),
    orm.select({ animeId: castCreditsTable.animeId, count: count() })
      .from(castCreditsTable).groupBy(castCreditsTable.animeId),
    orm.select({
      ...
      .leftJoin(searchMemoryTable, and(
    ...
    orm.select({ id: accountsTable.id, ownerType: accountsTable.ownerType, ownerId: accountsTable.ownerId })
      .from(accountsTable).where(eq(accountsTable.verified, true)),
    orm.select({ animeId: workCreditsTable.animeId, personId: workCreditsTable.personId }).from(workCreditsTable),
    orm.select({ animeId: castCreditsTable.animeId, personId: castCreditsTable.personId }).from(castCreditsTable),
    orm.select({ animeId: researchSourcesTable.animeId, count: count() }).from(researchSourcesTable)
      .where(eq(researchSourcesTable.enabled, true)).groupBy(researchSourcesTable.animeId),
    orm.select({ animeId: eventsTable.animeId, count: count() }).from(eventsTable)
      .where(eq(eventsTable.verified, true)).groupBy(eventsTable.animeId),
    orm.select({ animeId: mediaItemsTable.animeId, count: count() }).from(mediaItemsTable)
      .where(publicMedia(db)).groupBy(mediaItemsTable.animeId),
    orm.select({ animeId: discussionAnimeTable.animeId, count: count() })
  ...
  for (const account of accounts) {
    const animeIds = account.ownerType === "anime"
      ? [account.ownerId]
