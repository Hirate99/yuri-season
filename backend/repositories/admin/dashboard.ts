...
export async function readHeldCandidates(db: D1Database): Promise<FeedCandidate[]> {
  ...
  const [evidence, reviews] = await Promise.all([
    orm
      .select({ candidateId: candidateEvidenceTable.candidateId, count: count() })
    ...
    orm
      .select({
        candidateId: reviewDecisionsTable.candidateId,
        reasonsJson: reviewDecisionsTable.reasonsJson,
      })
...
export async function readSources(db: D1Database): Promise<SourceHealth[]> {
  return database(db)
    ...
    .orderBy(
      desc(researchSourcesTable.enabled),
      desc(researchSourcesTable.failureCount),
...
export async function readJobs(db: D1Database): Promise<UpdateJob[]> {
  return database(db)
    ...
    .orderBy(desc(updateJobsTable.createdAt))
    .limit(30);
}
...
export async function readWorks(db: D1Database) {
  return (await readAllAnimeSummaries(db)).map((row) => ({
    ...mapAnime(row),
