...
async function assertAnimeLinks(db: D1Database, animeIds: string[]): Promise<void> {
  const row = await database(db)
    ...
    .from(animeTable)
    .where(inArray(animeTable.id, animeIds))
    .get();
...
function linkQueries(db: D1Database, discussionId: string, animeIds: string[]) {
  ...
  return animeIds.map((animeId) =>
    orm.insert(discussionAnimeTable).values({ discussionId, animeId }).onConflictDoNothing(),
  );
}
...
export async function createDiscussion(
  ...
  const existing = await orm
    ...
    .where(eq(discussionsTable.url, value.url))
    .get();
...
export async function updateDiscussion(
  ...
  const queries: BatchItem<"sqlite">[] = [
    ...
    orm.delete(discussionAnimeTable).where(
      and(
        eq(discussionAnimeTable.discussionId, id),
        // Preserve existing links while staying below D1's 100-parameter limit.
        notInArray(
          discussionAnimeTable.animeId,
          sql`(SELECT value FROM json_each(${JSON.stringify(animeIds)}))`,
        ),
      ),
    ),
    ...linkQueries(db, id, animeIds),
...
export async function unlinkDiscussionFromAnime(
  ...
  const remaining = await orm
    .select({ animeId: discussionAnimeTable.animeId })
  ...
  const [result] = await orm.batch([
    orm
      .delete(discussionAnimeTable)
      .where(
        and(eq(discussionAnimeTable.discussionId, id), eq(discussionAnimeTable.animeId, animeId)),
      ),
    orm
      .update(discussionsTable)
      .set({
...
export async function deleteDiscussionEverywhere(
  ...
  const discussion = await orm
    .select({ id: discussionsTable.id, title: discussionsTable.title, url: discussionsTable.url })
    .from(discussionsTable)
    .where(eq(discussionsTable.id, id))
    .get();
  ...
  await orm.batch([
    orm.insert(correctionsTable).select(
      orm
        ...
        .from(feedItemsTable)
        .where(and(eq(feedItemsTable.discussionId, id), isNull(feedItemsTable.withdrawnAt))),
    ),
    orm
    ...
    orm
      .update(feedItemsTable)
      .set({ discussionId: null })
      .where(eq(feedItemsTable.discussionId, id)),
