...
export async function updateEvent(db: D1Database, animeId: string, id: string, value: EventWrite, audit: ResourceChangeAudit): Promise<void> {
...
export async function deleteEvent(db: D1Database, animeId: string, id: string, audit: ResourceChangeAudit): Promise<D1Result> {
  const before = await resourceAuditSnapshot(db, animeId, "event", id);
  const orm = database(db);
