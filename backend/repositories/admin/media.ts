...
export async function updateMedia(db: D1Database, animeId: string, id: string, value: MediaWrite, audit: ResourceChangeAudit): Promise<void> {
...
export async function deleteMedia(db: D1Database, animeId: string, id: string, audit: ResourceChangeAudit): Promise<D1Result> {
  const before = await resourceAuditSnapshot(db, animeId, "media", id);
  const orm = database(db);
