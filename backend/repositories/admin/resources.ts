...
import {
  ...
  castSelection,
  staffSelection,
} from "~/infrastructure/db/read-models/detail";
import {
  accountsTable,
  animeTable,
  broadcastSlotsTable,
...
export async function readAdminAnimeResources(
  ...
  const [[animeRows, broadcasts, staff, cast, sources], content] = await Promise.all([
    orm.batch([
      orm
        .select({ id: animeTable.id, title: animeTable.titleZh })
        .from(animeTable)
      ...
      orm
        ...
        .orderBy(
          ...
          researchSourcesTable.label,
          researchSourcesTable.id,
        ),
    ]),
  ...
  const personIds = orm
    .select({ id: workCreditsTable.personId })
    .from(workCreditsTable)
    .where(eq(workCreditsTable.animeId, animeId))
    .unionAll(
      orm
        .select({ id: castCreditsTable.personId })
        .from(castCreditsTable)
        .where(eq(castCreditsTable.animeId, animeId)),
    );
  ...
  const accounts = accountRows
    .map(({ createdAt: _createdAt, ...account }) => ({
      ...account,
