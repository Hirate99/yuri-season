...
function trackUpsert(db: D1Database, value: ThemeSongWrite) {
  return database(db)
    ...
    .onConflictDoUpdate({
      target: [musicTracksTable.title, musicTracksTable.artist],
      set: {
...
export async function createThemeSong(
  db: D1Database,
  animeId: string,
  ...
  const insert = orm.insert(animeThemeSongsTable).values({
    ...
    trackId:
      ...
      sql`(${orm
        ...
        .from(musicTracksTable)
        .where(
          and(eq(musicTracksTable.title, value.title), eq(musicTracksTable.artist, value.artist)),
        )})`,
...
export async function upsertVerifiedThemeSongFromBatch(
  ...
  if (existing.title !== value.title || existing.artist !== value.artist) {
    throw new HttpError(
      409,
      `${value.songKind.toUpperCase()}${value.sequence} 与现有曲目冲突，需要人工核对。`,
    );
  }
...
export async function updateThemeSong(
  db: D1Database,
  animeId: string,
  id: string,
  value: ThemeSongWrite,
...
export async function deleteThemeSong(
  db: D1Database,
  animeId: string,
  id: string,
  audit: ResourceChangeAudit,
