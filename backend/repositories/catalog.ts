...
import {
  readCatalogAnimeForSeason,
  type CatalogAnimeRecord,
} from "~/infrastructure/db/read-models/anime";
...
async function catalogForSeason(
  db: D1Database,
  season: Season,
  ...
  return {
    season,
    anime: rows.map(mapCatalogAnime),
    events:
      options.events === "today"
