...
export function sourceCheckQuery(
  db: D1Database,
  check: Omit<SourceCheckWrite, "checkedAt"> & { checkedAt?: string },
) {
  const checkedAt = check.checkedAt ?? sql`CURRENT_TIMESTAMP`;

  return database(db)
    .update(researchSourcesTable)
    .set({
...
export async function recordSourceChecks(db: D1Database, checks: SourceCheckWrite[]) {
  ...
  const results = await orm.batch([
    sourceCheckQuery(db, first),
    ...rest.map((check) => sourceCheckQuery(db, check)),
  ]);
