...
function normalizedPlatformObjectId(
...
export async function prepareBatchCandidate(
  ...
  const common: CandidateDraft = {
    ...
    sourceIdentity:
      ...
      (source.trustLevel === "official"
        ? "official"
        : source.trustLevel === "verified_creator"
          ? "creator"
          : "community"),
  ...
  if (candidate.contentClass === "cast_post") {
    ...
    if (
      !account?.verified ||
      account.owner_type !== "person" ||
      account.owner_id !== candidate.personId
    ) {
    ...
    if (!platformObjectId)
      throw new HttpError(400, "A cast post requires a stable platform object ID.");
  ...
  if (candidate.contentClass === "fanwork") {
    ...
    if (candidate.url !== originalUrl)
      throw new HttpError(400, "The candidate URL must be the creator's original URL.");
  ...
  return {
    ...
    platformObjectId,
    originKey:
      candidate.originKey ?? (platformObjectId ? `source:${source.id}:${platformObjectId}` : null),
  };
}
...
export async function storeAccountDiscovery(
  ...
  const credit = await orm
    ...
    .where(
      and(
        eq(castCreditsTable.animeId, value.animeId),
        eq(castCreditsTable.personId, value.personId),
      ),
    )
    .get();

  if (!credit)
    throw new HttpError(400, "The discovered account owner has no cast credit for this work.");
  ...
  const fingerprint = await stableFingerprint(
    `account_identity|${value.personId}|${value.platform}|${value.url}`,
  );

  const priorClaim = await orm
  ...
  const existing = await orm
    ...
    .from(accountsTable)
    .where(eq(accountsTable.url, value.url))
    .get();
