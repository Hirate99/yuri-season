...
function isDeterministicCommunityCandidate(
  candidate: BatchCandidate,
  observation: BatchObservation,
) {
  if (candidate.contentClass !== "community_thread" || candidate.sourceIdentity !== "community")
    return false;

  if (observation.metadata?.bodyCopied !== false || observation.metadata?.originalOpened !== true)
    return false;
  ...
  return (
    observation.metadata?.hotMarker === true ||
...
export function decideBatchCandidate(
  candidate: BatchCandidate,
  source: SourceRecord,
  observation: BatchObservation,
) {
  ...
  if (candidate.contentClass === "editorial") {
    return {
      ...
      reasons: [...reasons, "Editorial and operational content is not public feed material."],
    };
  }
  ...
  if ((candidate.safetyRating ?? "unknown") !== "safe") {
    return {
      decision: "hold" as const,
      reasons: [...reasons, "The safety rating is not eligible for automatic publishing."],
    };
  }
  ...
  if ((candidate.spoilerLevel ?? "none") === "major") {
    return {
      decision: "hold" as const,
      reasons: [...reasons, "Major spoilers are not published automatically."],
    };
  }
  ...
  if ((candidate.presentationMode ?? "link_only") !== "link_only") {
    return {
      decision: "hold" as const,
      reasons: [...reasons, "Automated batches only support link-only presentation."],
    };
  }
  ...
  if (candidate.review.confidence < threshold) {
    return {
      decision: "hold" as const,
      reasons: [...reasons, `Confidence is below ${threshold}.`],
    };
  }
