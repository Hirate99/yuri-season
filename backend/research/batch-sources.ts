...
export async function readResolvedAccount(
  db: D1Database,
  accountId: string,
  ...
  return (
    (await database(db)
      ...
      .leftJoin(
        peopleTable,
        and(eq(accountsTable.ownerType, "person"), eq(peopleTable.id, accountsTable.ownerId)),
      )
      .leftJoin(
        animeTable,
        and(eq(accountsTable.ownerType, "anime"), eq(animeTable.id, accountsTable.ownerId)),
      )
...
async function ensureAccountSource(
  db: D1Database,
  ...
  const existing = await orm
    .select({ id: researchSourcesTable.id })
    .from(researchSourcesTable)
    .where(
      and(
        eq(researchSourcesTable.accountId, account.id),
        eq(researchSourcesTable.sourceType, "social"),
      ),
    )
  ...
  const urlMatch = await orm
    .select({ id: researchSourcesTable.id, account_id: researchSourcesTable.accountId })
    .from(researchSourcesTable)
    .where(eq(researchSourcesTable.url, account.url))
    .get();

  if (urlMatch) {
    if (urlMatch.account_id !== account.id)
      throw new HttpError(409, "The account URL belongs to another source.");
...
async function ensureInlineSource(db: D1Database, input: BatchInlineSource): Promise<SourceRecord> {
  ...
  const existing = await orm
    .select({ id: researchSourcesTable.id })
    .from(researchSourcesTable)
    .where(eq(researchSourcesTable.url, input.url))
    .get();
...
export async function resolveObservationSource(
  db: D1Database,
  ...
  if (observation.accountId) {
    ...
    if (!account?.verified)
      throw new HttpError(400, "An account observation requires a verified account.");
