...
export async function planSourceJobs(
  db: D1Database,
  lane: JobLane,
  limit: number,
): Promise<number> {
  ...
  const due = await orm
    ...
    .where(
      and(
        ...
        or(
          isNull(researchSourcesTable.nextCheckAt),
          lte(researchSourcesTable.nextCheckAt, sql`CURRENT_TIMESTAMP`),
        ),
        or(
          isNull(researchSourcesTable.leaseUntil),
          lt(researchSourcesTable.leaseUntil, sql`CURRENT_TIMESTAMP`),
        ),
      ),
    )
    .orderBy(
      desc(
        sql`CASE WHEN ${researchSourcesTable.urgencyUntil} > CURRENT_TIMESTAMP THEN 1 ELSE 0 END`,
      ),
  ...
  const queries = due.map((source, index) =>
    orm
      .insert(updateJobsTable)
      .values({
        id: createId("job"),
...
export async function startJob(db: D1Database, job: UpdateJobRow): Promise<void> {
  const result = await database(db)
    ...
    .where(and(leaseWhere(job), eq(updateJobsTable.status, "leased")))
    .run();
...
export async function heartbeatJob(db: D1Database, job: UpdateJobRow): Promise<void> {
  const result = await database(db)
    ...
    .where(and(leaseWhere(job), eq(updateJobsTable.status, "running")))
    .run();
