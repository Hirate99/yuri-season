...
export async function sourceHasBaseline(db: D1Database, sourceId: string): Promise<boolean> {
  const row = await database(db).select({ id: sourceObservationsTable.id }).from(sourceObservationsTable)
    .where(eq(sourceObservationsTable.sourceId, sourceId)).get();
  return Boolean(row);
}
...
export async function storeObservation(
  ...
  const existing = await orm.select({ id: sourceObservationsTable.id }).from(sourceObservationsTable).where(and(
  ...
  const result = await orm.insert(sourceObservationsTable).values({
    id,
    sourceId: source.id,
...
export async function storeClaim(
  ...
  const orm = database(db);
  const existing = await orm.select({ id: claimsTable.id }).from(claimsTable)
