...
- 不建立通用网页搜索引擎。
- 不无差别镜像第三方正文或图片。官网和已验证来源中用于公开 Feed 的图片必须按媒体管线转存、保留原始出处并回读验证；只有来源或平台的明确规则禁止转载、再托管或嵌入时才使用 `link_only_policy`。
- 不绕过平台登录、反爬、付费墙或访问控制。
...
频繁、便宜、确定性高。默认日常更新只检查已登记且启用的公式 NEWS / Feed / API，以及已经验证并启用的作品公式、声优、原作者 / 创作者和署名制作组 X 账号时间线。四类账号都是一等更新通道，不能因已有公式号而省略人物账号。Bangumi、社区串和其他登记来源可由各自的维护任务处理，但不得因此把一次普通“更新网站”扩展成全季发现。

主要产出：

- 页面或 API 的增量观察。
- 播出、配信、Staff / Cast、活动和已知串状态变更。
- 已验证声优、原作者 / 创作者和制作组账号中的作品相关贺图、出演动态、评论、制作笔记与署名创作。

若已验证 X 账号暂时没有可登记的 API、RSS 或稳定页面来源，日常更新通过登录态浏览器按已提交游标做增量检查，不把账号主页交给通用抓取器。找到的内容必须回到原始帖核验。账号发现、身份扩展和缺失账号验证不属于这个闭环，只有用户明确指定作品、人物或平台范围时才进入 `account-discovery`。

### 2.2 Discovery agent

按需、预算受控，用于发现此前未知的对象：新作品、角色生日来源、作者贺图、同人作品和社区集中讨论串。新账号及身份验证使用独立、显式且限定实体或平台范围的 `account-discovery`，普通 Discovery 不自动扩张到账号发现。
...
确认变化后先计算影响面，不把“生成了一条 Feed”视为完成。执行端读取受影响实体的当前结构化状态，把同一证据支持的资料字段、事件、媒体、主题曲、外部动作和公开投影作为一个幂等对账单处理；缺失或过期的支持项必须补齐，证据不支持的字段保持为空，冲突项停止自动写入。对账完成后重新读取资源并验证读者实际看到的结果。重复出现的领域对账必须编码为确定性脚本与测试，而不是继续扩写 prompt 或 skill 特例。
...
- 与已验证生日匹配的官方 / 创作者庆祝内容按对应分轨规则自动发布（生日事件本身不进情报流）。
...
- 每天为未来 30 天已验证生日生成或确认 recurring event（生日只进角色资料与日历事件，不进情报流）。
- 生日前 7 天开始搜索官方活动、作者 / Cast 贺图和同人标签；找到的庆祝内容按官方贺图、创作者贺图或声优帖各自的分轨规则进入情报流。
- 没有可靠来源的生日不展示具体日期。
...
- 保存作者、原链接、平台分级、角色关联和媒体处理依据。`link_only_policy` 只用于存在可引用的明确转载、再托管或嵌入禁令；上传能力缺失或处理失败必须 hold，不能降级成 link-only。
...
- 日常通过登录态浏览器增量检查已验证的公式、声优、原作者 / 创作者和制作组 X 账号；需要交互时先用应用内部浏览器，再用用户 Chrome，并分别复用一个专用 tab。
- 仅在显式 Discovery 中使用搜索引擎、Pixiv、贴吧 / NGA / 百合会等来源；账号查找与交叉验证还要求显式、限定范围的 `account-discovery`。
