...
if (import.meta.main) {
  ...
  if (!inputPath || !originalR2Key || !outputDirectory) {
    throw new Error(
      "Usage: bun run research:media:variants -- <input> <original-r2-key> <output-directory>",
    );
  }

  console.log(
    JSON.stringify(await createMediaVariants(inputPath, originalR2Key, outputDirectory), null, 2),
  );
}
