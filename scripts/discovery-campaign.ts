...
async function loadCampaign(campaignPath: string, expectedProfile: string): Promise<DiscoveryCampaign> {
  ...
  if (campaign.profile && campaign.profile !== expectedProfile) {
    throw new Error(`active ${campaign.profile} campaign does not match requested ${expectedProfile} profile`);
  }
  return campaign;
}
...
async function lease(arguments_: string[], campaignPath: string, profile: string): Promise<void> {
  ...
  const queries = leaseCampaignQueries(campaign, batchLimit(requestedLimit), new Date(), (query) =>
...
async function record(path: string | undefined, campaignPath: string, profile: string): Promise<void> {
...
async function cancel(scopeType: string | undefined, scopeId: string | undefined, reason: string | undefined, campaignPath: string, profile: string): Promise<void> {
...
const profileArgument = arguments_.find((argument) => argument.startsWith("--profile="))?.slice("--profile=".length);
...
else if (command === "cancel") await cancel(positional[0], positional[1], positional.slice(2).join(" "), campaignPath, profile);
else if (command === "status") process.stdout.write(JSON.stringify(campaignSummary(await loadCampaign(campaignPath, profile)), null, 2));
