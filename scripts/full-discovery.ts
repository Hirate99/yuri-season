...
if ((await priorFile.exists()) && !replace) {
  ...
  if (
    ...
    throw new Error(
      "active discovery campaign still has unfinished queries; use campaign status/next or --replace",
    );
  }
}
...
const byKind = Object.fromEntries(
  [...new Set(queries.map((query) => query.searchKind))]
    .sort()
    .map((kind) => [kind, queries.filter((query) => query.searchKind === kind).length]),
);
