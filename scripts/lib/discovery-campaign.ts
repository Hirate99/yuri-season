...
export function createCampaign(
  input: Omit<
    DiscoveryCampaign,
    "schemaVersion" | "campaignId" | "updatedAt" | "mode" | "queries"
  > & {
...
function validateResult(query: CampaignQuery, result: DiscoveryResult): void {
  ...
  if (
    query.completionPolicy.allowedCompleteSurfaces.length > 0 &&
  ...
  if (query.completionPolicy.recordEveryOriginal) {
    const stableHits = result.hits.filter(
      (hit) =>
        typeof hit.metadata?.platformObjectId === "string" &&
        hit.metadata.platformObjectId.length > 0,
    );

    if (stableHits.length < coverage.originalPostsInspected) {
      throw new Error(
        `${query.operation} ${result.queryId} must record every inspected original with a stable platformObjectId`,
      );
    }
  }
}
...
function cursorForResult(query: CampaignQuery, result: DiscoveryResult): Record<string, unknown> {
  ...
  const committed = {
    ...
    lastUsefulHits: result.hits.filter(
  ...
  return {
    ...committed,
    activeTerms: [
      ...new Set(result.discoveredTerms.map((item) => item.term.trim()).filter(Boolean)),
    ].slice(0, 50),
...
function nextSearchAt(query: CampaignQuery, result: DiscoveryResult): string {
  ...
  const requested = validTimestamp(result.nextCheckAt)
    ? Date.parse(result.nextCheckAt)
    : latestAllowed;
...
export function cancelCampaignQueries(
  ...
  const matches = campaign.queries.filter(
    (query) =>
...
export async function memoryRecordsForResults(
  ...
  return Promise.all(
    results.map(async (result) => {
      if (resultIds.has(result.queryId))
        throw new Error(`duplicate query result ${result.queryId}`);
