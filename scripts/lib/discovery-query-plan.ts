...
export type DiscoveryQuery = {
  ...
  knownHits: Array<
    Pick<SearchMemoryHitSummary, "canonicalUrl" | "title" | "outcome" | "lastSeenAt">
  >;
};
...
function hasCommunity(
  ...
  community: "yamibo" | "tieba" | "moesen" | "nga",
) {
...
function hasWorkAccount(resources: AdminAnimeResources, animeId: string) {
  return resources.accounts.some(
    (account) => account.ownerType === "anime" && account.ownerId === animeId,
  );
}
...
export function buildDiscoveryPlan(input: PlanInput): DiscoveryQuery[] {
  ...
  function add(query: DiscoveryQueryInput) {
    ...
    if (!input.force && input.profile !== "social-audit" && !isDue(prior, input.now, operation))
      return;
  ...
  add({
    scopeType: "season",
    scopeId: input.seasonId,
    animeId: null,
    animeTitle: null,
    searchKind: "catalog",
    targetKey: "season:yuri-catalog",
    queryText: `\"${input.seasonLabel}\" 百合 アニメ 新作 公式`,
    priority: 5,
  ...
  const yamiboAliases = [
    ...new Set(
      input.anime
        .flatMap((anime) => [anime.titleZh, anime.titleJa, anime.titleEn])
  ...
  add({
    ...
    targetKey: "community:yamibo-recent",
    ...
    priority: 5,
    reason:
      "Incrementally scan the Yamibo animation-board thread list instead of relying on indexed exact-title or 专楼 searches; deduplicate canonical thread URLs against database resources and knownHits, then retain dedicated threads and materially active relevant discussions",
  });
  add({
    scopeType: "season",
    scopeId: input.seasonId,
    ...
    animeTitle: null,
    searchKind: "catalog",
    targetKey: "season:relationship-catalog",
    queryText: `\"${input.seasonLabel}\" アニメ 女子 関係性 オリジナル`,
    priority: 4,
  ...
  for (const anime of input.anime) {
    ...
    if (input.force || !hasWorkAccount(resources, anime.id)) {
      add({
        ...common,
        searchKind: "social",
        targetKey: "social:work",
        queryText: `\"${titleJa}\" 公式 X キャスト スタッフ`,
        priority: 4,
        stage: "people",
        platform: "X",
    ...
    if (input.force || !resources.themeSongs.some((song) => song.verified)) {
      add({
        ...common,
        searchKind: "official_news",
        targetKey: "music:theme-songs",
        queryText: `"${titleJa}" OP ED 主題歌 オープニング エンディング 公式`,
        priority: 5,
        reason:
          "从动画官网、唱片公司或官方音乐页面补齐主题曲、制作名单与页面已有的唱片封面；只接收明确字段",
      });
    } else {
      const incompleteSongs = resources.themeSongs.filter(
        (song) =>
          song.verified &&
      ...
      if (incompleteSongs.length > 0) {
        ...
        add({
          ...common,
          searchKind: "official_news",
          targetKey: "music:theme-song-projection",
          ...
          priority: 4,
          reason:
            "对账已验证主题曲的完整用户投影：曲目身份仍由第一方来源证明，精确 Apple Music 曲目补试听入口与封面；不存在或歧义时不猜测",
        });
      }
    }
    ...
    for (const account of resources.accounts) {
      if (!account.verified) {
        ...
        const eligible =
          account.ownerType === "anime" || cast.some((item) => item.isMainGroup) || Boolean(staff);
        ...
        add({
          ...
          contentLane:
            cast.length > 0 ? "cast" : account.ownerType === "anime" ? "official" : "creator",
      ...
      const prior = remembered.get(
        memoryKey(target.scopeType, target.scopeId, "social", target.targetKey),
      );
      ...
      add({
        ...
        completionPolicy: target.timelineMode
          ...
          : undefined,
        queryText: accountUpdateQuery(
          account,
          titleJa,
          prior?.searchedAt ?? null,
          input.now,
          target.timelineMode,
        ),
        reason:
          target.timelineMode === "project_persona"
    ...
    const officialXAccounts = resources.accounts.filter(
      (account) =>
        account.verified &&
    ...
    if (officialXAccounts.length > 0) {
        ...
        ...new Set(
          [
            ...
            ...(Array.isArray(prior?.cursor?.activeTerms)
              ? prior.cursor.activeTerms.filter(
      ...
      add({
        ...
        contentLane: "official",
        reason:
          "持续发现每部当季作品正在使用的 X 官方标签、动画标签、企划简称及主角组名称；标签来自已验证官方资料与近期官方原帖，所有命中必须回到原帖并与账号监控共享 stable post ID 去重",
      });
    }
    ...
    add({
      ...common,
      searchKind: "media",
      targetKey: "media:creator-art",
      queryText: `\"${titleJa}\" 描き下ろし 応援イラスト`,
      priority: 4,
    ...
    add({
      ...
      contentLane: "fanwork",
      reason:
        "用 Pixiv 免登录公开标签接口按新序检查同人；逐条记录已见 artwork ID，只对原作者原帖建候选（默认 hold）",
    });
    add({
      ...common,
      searchKind: "media",
      targetKey: "media:fanwork:x",
    ...
    add({
      ...common,
      searchKind: "media",
      targetKey: "media:fanwork:instagram",
    ...
    for (const community of communities) {
      ...
      add({
        ...common,
        searchKind: "community",
        targetKey: `community:${community.key}`,
    ...
    const knownPlatforms = new Set(
      resources.accounts.map((item) => `${item.ownerId}:${item.platform.toLowerCase()}`),
    );
    ...
    for (const staff of resources.staff) {
      ...
      add({
        scopeType: "person",
        scopeId: staff.personId,
        animeId: anime.id,
        animeTitle: anime.titleZh,
        searchKind: "social",
        targetKey: "account:official-x",
    ...
    for (const cast of resources.cast) {
      ...
      if (input.includeBirthdays === true && cast.isMainGroup && !cast.birthdayVerified) {
        ...
        add({
          ...
          queryText: `\"${character}\" 誕生日 公式 \"${titleJa}\"`,
          priority: 4,
      ...
      for (const accountPlatform of accountPlatforms) {
        if (knownPlatforms.has(`${cast.personId}:${accountPlatform.platform.toLowerCase()}`))
          continue;

        add({
          scopeType: "person",
          scopeId: cast.personId,
          animeId: anime.id,
          animeTitle: anime.titleZh,
          searchKind: "social",
          targetKey: `account:official-${accountPlatform.key}`,
          queryText: `\"${person}\" 公式 ${accountPlatform.platform} \"${titleJa}\"`,
          priority: 3,
          personId: cast.personId,
          characterIds: resources.cast
            .filter((item) => item.personId === cast.personId)
            .map((item) => item.characterId),
          platform: accountPlatform.platform,
          contentLane: "cast",
  ...
  for (const query of planned.values()) {
    if (query.searchKind !== "social" || query.platform?.toLowerCase() !== "x" || !query.animeId)
      continue;
    ...
    query.knownHits = [
      ...new Map(
        [...query.knownHits, ...relatedHits].map((hit) => [hit.canonicalUrl, hit]),
      ).values(),
    ];
  }

  const isAccountDiscovery = (query: DiscoveryQuery) =>
  ...
  const matchesProfile = (query: DiscoveryQuery) => {
    ...
    if (input.profile === "social-audit") {
      return (
        !isAccountDiscovery(query) && query.searchKind === "social" && query.socialAuditEligible
      );
    }
  ...
  const stageRank: Record<DiscoveryStage, number> = {
    sources: 0,
    official: 1,
    tags: 2,
    people: 3,
  ...
  const selected = [...planned.values()].filter(
    (query) => matchesProfile(query) && matchesRequestedScope(query),
  );

  return selected
    .sort(
      (a, b) =>
        (input.profile === "social-audit" ? stageRank[a.stage] - stageRank[b.stage] : 0) ||
