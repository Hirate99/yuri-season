...
export async function searchRecords(result: FullSyncResult): Promise<SearchMemoryWrite[]> {
  return Promise.all(
    result.sources.map(async (source) => ({
      ...
      targetKey: source.url,
      queryText: source.url,
      status: "active",
      cursor: {},
      lastResultHash: await stableFingerprint(
        source.items.map((item) => item.contentHash).join("|"),
      ),
      lastResultCount: source.items.length,
      usefulResultCount: 0,
      ...
      hits: source.items.map((item) => ({
        ...
        outcome: "seen",
        metadata: { sourceItemId: item.sourceItemId, publishedAt: item.publishedAt },
      })),
    })),
  );
}
