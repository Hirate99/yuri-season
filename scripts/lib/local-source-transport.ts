...
async function fetchWithResolvedAddress(url: string, init: RequestInit): Promise<Response> {
  ...
  if (target.protocol !== "https:" || target.port)
    throw new Error("DNS fallback only allows standard HTTPS sources");
  ...
  if (exitCode !== 0)
    throw new Error(`DNS fallback failed: ${stderr.trim() || `curl exit ${exitCode}`}`);

  const parsed = responseParts(new Uint8Array(stdoutBuffer));

  const body =
    parsed.status === 204 || parsed.status === 304 ? null : Uint8Array.from(parsed.body).buffer;
