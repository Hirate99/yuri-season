...
async function lease(): Promise<void> {
  ...
  const { jobs } = await rpcData(
    adminApi().api.admin.jobs.lease.$post({ json: { owner, limit: limitValue } }),
  );
...
async function heartbeat(id: string): Promise<void> {
  ...
  const result = await rpcData(
    adminApi().api.admin.jobs[":id"].heartbeat.$post({
      param: { id },
  ...
  await Bun.write(
    leasePath(id),
    JSON.stringify({ ...stored, leaseUntil: result.leaseUntil }, null, 2),
  );
...
async function finish(id: string, outcome: "completed" | "partial" | "failed"): Promise<void> {
  ...
  const result = await rpcData(
    adminApi().api.admin.jobs[":id"].complete.$post({
      param: { id },
      json: {
        ...
        result: {
          ...
          finishedBy: process.env.YURI_AGENT_ID?.trim() || "codex-local",
        },
      },
    }),
  );
