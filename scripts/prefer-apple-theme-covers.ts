...
export function selectExactAppleTrack(
  ...
  if (unique.length > 1)
    return { status: "ambiguous", trackIds: unique.map((track) => String(track.trackId)) };
...
async function lookupAppleTrack(id: string): Promise<AppleTrack> {
  const response = await fetch(
    `https://itunes.apple.com/lookup?id=${encodeURIComponent(id)}&country=jp&entity=song`,
  );
  ...
  const track =
    payload.results.find((item) => item.wrapperType === "track" && String(item.trackId) === id) ??
...
async function searchAppleTrack(
  song: Pick<AdminThemeSong, "title" | "artist">,
): Promise<AppleTrackResolution> {
  const term = encodeURIComponent(`${song.title} ${song.artist}`);

  const response = await fetch(
    `https://itunes.apple.com/search?term=${term}&country=jp&media=music&entity=song&limit=25`,
  );
...
export async function preferAppleThemeCovers(apply: boolean): Promise<void> {
  ...
  for (const item of anime) {
    ...
    for (const song of resources.themeSongs) {
      ...
      if (
        song.officialUrl === officialUrl &&
        song.coverUrl === coverUrl &&
        song.coverSourceUrl === officialUrl
      ) {
        summary.unchanged += 1;
        continue;
      }
      ...
      summary.changed += 1;
      console.log(
        `${apply ? "updated" : "would update"}: ${song.title} — ${apple.artistName ?? song.artist}`,
      );
    }
  }
