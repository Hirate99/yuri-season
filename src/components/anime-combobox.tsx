...
export function AnimeCombobox({
  ...
  const filtered = useMemo(() => {
    ...
    const matchingAnime = anime.filter((item) =>
      [item.titleZh, item.titleJa, item.titleEn].some((title) =>
    ...
    <div className="relative min-w-0">
      ...
      >
        {compact && (
          <ListFilter
            size={18}
            className={value ? "text-accent md:hidden" : "text-muted md:hidden"}
            aria-hidden="true"
          />
        )}
        {compact && value && (
          <span
            className="absolute top-1 right-1 size-1.5 rounded-full bg-accent md:hidden"
            aria-hidden="true"
          />
        )}
        <span
          className={`min-w-0 truncate text-sm ${compact ? "hidden md:block" : "flex-1 text-left"}`}
        >
          {selected.label}
        </span>
        <ChevronDown
          ...
          <ul className="max-h-[min(18rem,calc(100dvh-17rem))] overflow-y-auto pt-1.5">
            {filtered.map((item, index) => {
              ...
              return (
                ...
                >
                  <Check
                    className={`shrink-0 text-[#786bd1] ${active ? "visible" : "invisible"}`}
                    size={14}
                  />
            ...
            {filtered.length === 0 && (
              <li className="px-3 py-6 text-center text-xs text-muted">没有匹配作品</li>
            )}
          </ul>
        </div>
      )}
