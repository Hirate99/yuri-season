...
function AccountLink() {
  ...
  return (
    <Link
      to="/account"
      title={name}
      className="max-w-28 truncate justify-self-end rounded-full bg-raised px-3 py-2 text-xs font-semibold text-muted hover:text-accent sm:max-w-44"
    >
      {name}
    </Link>
  );
      ...
      <div className="min-h-screen">
        <header className="sticky top-0 z-30 h-15 border-b border-black/[0.05] bg-white/80 backdrop-blur-2xl md:h-16">
          <div className="mx-auto grid h-full w-[calc(100%-1.5rem)] max-w-[1320px] grid-cols-[1fr_auto] items-center md:w-[calc(100%-3rem)] md:grid-cols-[1fr_auto_1fr]">
            <Link
              to="/"
              className="inline-flex w-max items-center gap-2 text-[15px] font-semibold tracking-[-0.02em] text-[#25242b]"
              aria-label="YuriSeason 首页"
            >
              <BrandMark className="size-7 text-[#786bd1]" />
              <span>YuriSeason</span>
            </Link>
            <Navigation />
            <AccountLink />
          </div>
