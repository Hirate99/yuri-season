...
export function BroadcastTime({
  ...
  const local =
    effectiveTimeZone && effectiveTimeZone !== slot.timezone
  ...
  return (
    <span className={align === "end" ? "block text-right" : "block"}>
      <strong className="block text-sm tabular-nums">
        {showWeekday && (
          <span className="mr-2 text-xs font-normal text-muted">{weekdayLabel(slot.weekday)}</span>
        )}
        {slot.localTime}{" "}
        <small className="font-medium text-muted">{timeZoneLabel(slot.timezone)}</small>
      </strong>
