...
const card =
  "flex min-w-0 items-start gap-2.5 rounded-lg border border-line bg-white p-3 shadow-[0_2px_6px_rgba(37,35,43,0.03)] transition-colors hover:border-accent/30";

const label =
  "mb-1.5 inline-flex rounded bg-accent-soft/70 px-1.5 py-0.5 text-xs font-medium text-accent";
...
export function TodayPanel({
  ...
  return (
    <section
      className="mt-4 rounded-2xl bg-accent-soft/55 p-4 md:p-6"
    ...
    >
      <header className="mb-4 flex items-center justify-between gap-3">
        ...
        <Link
          to="/calendar"
          className="inline-flex shrink-0 items-center gap-1 py-1 text-xs font-medium text-accent hover:underline"
        >
          日历
          <ArrowRight size={14} />
        </Link>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[repeat(auto-fill,250px)]">
        {airingToday.map((anime) => (
          <Link key={anime.id} to="/anime/$slug" params={{ slug: anime.slug }} className={card}>
            <CoverImage
              className="aspect-[3/4] w-10 shrink-0 rounded"
          ...
          const content = (
            <>
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1.5">
                  <EventTime event={event} viewerTimeZone={viewerTimeZone} showTime />
                </div>
              </div>
              {portrait && (
                <CoverImage
      ...
      {!airingToday.length && !events.length && (
        <p className="py-2 text-sm text-muted">今天暂无放送或活动。</p>
      )}
    </section>
  );
}
