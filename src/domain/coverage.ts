...
export function coverageChecks(item: AdminAnimeCoverage): Check[] {
  return [
    ...
    {
      label: "主角团",
      value: `${item.mainCharacters}/${item.mainCharacterExpected ?? "?"}`,
      ready:
        item.mainCharacterExpected !== null && item.mainCharacters === item.mainCharacterExpected,
      core: true,
    },
    {
      label: "角色来源",
      value: String(item.sourcedMainCharacters),
      ready: item.mainCharacters > 0 && item.sourcedMainCharacters === item.mainCharacters,
      core: true,
    },
    {
      label: "中文名来源",
      value: String(item.namedMainCharacters),
      ready: item.mainCharacters > 0 && item.namedMainCharacters === item.mainCharacters,
    },
    {
      label: "生日核验",
      value: `${item.auditedMainBirthdays}/${item.mainCharacters}`,
      ready: item.mainCharacters > 0 && item.auditedMainBirthdays === item.mainCharacters,
    },
    { label: "已公开生日", value: String(item.verifiedMainBirthdays), ready: true },
    {
      label: "账号",
      value: String(item.verifiedAccounts),
      ready: item.verifiedAccounts > 0,
      core: true,
    },
    ...
    {
      label: "歌曲封面",
      value: String(item.themeSongCovers),
      ready: item.themeSongs > 0 && item.themeSongCovers === item.themeSongs,
    },
