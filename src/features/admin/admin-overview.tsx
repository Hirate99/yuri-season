...
export type AdminArea = "overview" | "review" | "works" | "coverage" | "automation" | "seasons";

export function AdminOverview({ data, onNavigate }: { data: AdminDashboard; onNavigate: (area: AdminArea) => void }) {
  ...
  const incompleteWorks = currentCoverage.filter((item) => !item.hasCover || item.broadcasts === 0 || item.staff === 0
    || item.cast === 0 || item.verifiedAccounts === 0 || item.sources === 0 || item.themeSongs === 0
    || item.themeSongCovers < item.themeSongs || item.verifiedEvents === 0 || item.discussions === 0 || item.media === 0).length;
  ...
  const actions = [
    { label: "待复核", count: data.counts.held, detail: "候选动态", area: "review" as const, Icon: Inbox },
    { label: "来源异常", count: sourceErrors, detail: "抓取来源", area: "automation" as const, Icon: RadioTower },
    { label: "资料未齐", count: incompleteWorks, detail: currentSeason?.label ?? "当前季度", area: "coverage" as const, Icon: Database },
    { label: "运行异常", count: failedRuns + blockedJobs, detail: "批次与任务", area: "automation" as const, Icon: AlertCircle },
  ];
    ...
    <div className="grid gap-7">
      <section>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {actions.map(({ label, count, detail, area, Icon }) => (
            <button className="group rounded-3xl border border-black/[0.06] bg-white p-5 text-left shadow-[0_12px_40px_rgba(15,23,42,0.045)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_50px_rgba(15,23,42,0.08)]" key={label} onClick={() => onNavigate(area)}>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-3xl border border-black/[0.06] bg-white p-5 shadow-[0_12px_40px_rgba(15,23,42,0.04)]">
          <div className="flex items-center justify-between"><h3 className="text-sm font-bold">最近运行</h3><button className="text-[10px] font-semibold text-[#665ab1]" onClick={() => onNavigate("automation")}>全部</button></div>
