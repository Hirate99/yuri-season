  ...
  return (
    <fieldset className="grid gap-2 md:col-span-2">
      ...
      <div className="flex flex-wrap gap-1.5">
        <button
          className="rounded-full bg-[#eeeafd] px-2.5 py-1.5 text-[9px] font-semibold text-[#51459d]"
          type="button"
          onClick={() =>
            select(anime.filter((work) => work.seasonId === currentSeasonId).map((work) => work.id))
          }
        >
          全选当前季度
        </button>
        <button
          className="rounded-full bg-white px-2.5 py-1.5 text-[9px] font-semibold text-muted"
          type="button"
          onClick={() => select(anime.map((work) => work.id))}
        >
          全选全部作品
        </button>
        <button
      ...
      <div className="grid max-h-52 gap-1 overflow-y-auto rounded-xl border border-black/[0.06] p-1.5 sm:grid-cols-2">
        {anime.map((work) => {
          ...
          return (
            <label
              className={`${hidden ? "hidden" : "flex"} cursor-pointer items-start gap-2 rounded-lg px-2.5 py-2 text-[10px] hover:bg-[#f4f5f7]`}
              key={work.id}
            >
              <input
                type="checkbox"
                checked={selected.includes(work.id)}
                disabled={work.id === currentAnimeId}
                onChange={() => toggle(work.id)}
              />
              <span className="min-w-0">
                <strong className="block truncate font-semibold">{work.titleZh}</strong>
                <small className="mt-0.5 block truncate text-[9px] text-muted">
                  {work.seasonLabel}
                  {work.id === currentAnimeId ? " · 当前作品" : ""}
                </small>
              </span>
            </label>
          );
...
function DiscussionForm({
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<z.input<typeof discussionSchema>, unknown, DiscussionWrite>({
    resolver: zodResolver(discussionSchema),
    defaultValues: {
      ...item,
      isActive: item?.isActive ?? true,
      animeIds: [...new Set([animeId, ...(item?.animeIds ?? [])])],
    },
  });
  ...
  const submit = handleSubmit((value) => {
    save.mutate({ kind: "discussion", value });
  });
    ...
    <form className="grid gap-3 pb-4 md:grid-cols-2" onSubmit={submit}>
      <AdminField label="平台">
        <input
          ...
          {...register("platform")}
          placeholder="作品贴吧 / 萌战吧 / NGA / 百合会"
          required
        />
      </AdminField>
      <AdminField label="最近活跃">
        <input
          className={adminInput}
          {...register("lastActivityAt", optionalText)}
          placeholder="带时区的 ISO 时间"
        />
      </AdminField>
      <AdminField label="标题" wide>
        <input className={adminInput} {...register("title")} required />
      </AdminField>
      <AdminField label="原始链接" wide>
        <input className={adminInput} {...register("url")} type="url" required />
      </AdminField>
      <AdminField label="备注" wide>
        <textarea className={adminInput} {...register("note", optionalText)} rows={2} />
      </AdminField>
      <WorkLinks
        anime={anime}
        currentAnimeId={animeId}
        selected={selected}
        onChange={(ids) => setValue("animeIds", ids, { shouldDirty: true })}
      />
        ...
        <div className="grid gap-2 rounded-xl border border-[#8b3048]/15 bg-[#fff7f9] p-3 md:col-span-2 sm:grid-cols-[minmax(0,1fr)_auto]">
          ...
          <button
            ...
            onClick={() => {
              if (
                !window.confirm(
                  `确认彻底删除这条讨论？它将从 ${item.sharedAnimeCount} 部作品及 Feed 撤下。`,
                )
              )
                return;
      ...
      <ResourceActions
        ...
        animeId={animeId}
        kind="discussion"
        id={item && item.sharedAnimeCount > 1 ? item.id : undefined}
        deleteLabel="从本作移除"
      />
    </form>
  );
  ...
  return (
    <section className="border border-line bg-raised px-4 xl:col-span-2">
      <div className="flex flex-wrap items-end justify-between gap-2 pt-4">
        <div>
          <h4 className="text-sm font-bold">集中讨论</h4>
          <p className="mt-1 text-[10px] text-muted">综合串只保存一份，可同时出现在多部作品页。</p>
        </div>
      </div>
      {items.map((item) => (
        <ResourceDetails
          key={item.id}
          title={item.title}
          meta={`${item.isActive ? item.platform : "停用"}${item.sharedAnimeCount > 1 ? ` · 跨 ${item.sharedAnimeCount} 部作品` : ""}`}
        >
          <DiscussionForm item={item} anime={anime} animeId={animeId} />
        </ResourceDetails>
      ))}
      <ResourceDetails title="新增讨论串" meta="＋">
        <DiscussionForm anime={anime} animeId={animeId} />
      </ResourceDetails>
    </section>
  );
}
