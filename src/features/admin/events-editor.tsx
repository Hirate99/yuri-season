...
function EventForm({
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<z.input<typeof eventSchema>, unknown, EventWrite>({
    ...
    <form className="grid gap-3 pb-4 md:grid-cols-2" onSubmit={submit}>
      ...
      <AdminField label="开始">
        <input
          ...
          {...register("startsAt", optionalText)}
          placeholder="2026-08-20T19:00:00+09:00"
        />
      </AdminField>
      <AdminField label="结束">
        <input className={adminInput} {...register("endsAt", optionalText)} placeholder="可空" />
      </AdminField>
      <AdminField label="时区">
      ...
      <AdminField label="重复">
        <input
          className={adminInput}
          {...register("recurrenceRule", optionalText)}
  ...
  return (
    <section className="border border-line bg-raised px-4">
      ...
      {resources.events.map((item) => (
        <ResourceDetails key={item.id} title={item.title} meta={item.startsAt ?? "时间待定"}>
          <EventForm item={item} resources={resources} animeId={animeId} />
        </ResourceDetails>
      ))}
      <ResourceDetails title="新增事件" meta="＋">
        <EventForm resources={resources} animeId={animeId} />
      </ResourceDetails>
    </section>
  );
}
