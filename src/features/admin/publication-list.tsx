        ...
        <article className="rounded-2xl bg-raised p-4.5" key={item.id}>
          <header className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <Badge tone={item.autoPublished ? "blue" : "neutral"}>
                  {item.autoPublished ? "自动" : "手动"}
                </Badge>
              ...
              <p className="mt-2 text-[10px] text-muted">
                {item.sourceName} · {dateTime(item.publishedAt)}
              </p>
            </div>
            <button
          ...
          {openId === item.id && (
            ...
            >
              ...
              />
              <button
                className="rounded-xl bg-charcoal px-4 py-2.5 text-xs font-bold text-white disabled:opacity-40"
                disabled={!reason.trim() || busyId === item.candidateId}
              >
                确认撤回
              </button>
            </form>
