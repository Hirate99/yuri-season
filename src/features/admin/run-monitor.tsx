            ...
            return (
              <article className="rounded-2xl bg-raised p-4.5" key={run.id}>
                <header className="flex items-start justify-between gap-4">
                  <div><strong className="text-xs">{dateTime(run.startedAt)}</strong><p className="mt-1 text-[9px] text-muted">{run.triggerType}</p></div>
                  <Badge tone={runTone(run.status)}>{run.status}</Badge>
                </header>
                {bits.length > 0 && <p className="mt-3 text-[10px] leading-5 text-muted">{bits.join(" · ")}</p>}
                <dl className="mt-4 grid grid-cols-4 gap-2 rounded-xl bg-white p-3">
                  {[["来源", run.sourceCount], ["观察", run.observationCount], ["候选", run.candidateCount], ["发布", run.publishedCount]].map(([label, value]) => (
                    <div key={label}><dt className="text-[8px] text-muted">{label}</dt><dd className="mt-1 text-xs font-bold">{value}</dd></div>
                  ))}
                </dl>
              </article>
            );
          ...
          {jobs.map((job) => (
            <article className="rounded-2xl bg-raised p-4" key={job.id}>
              <header className="flex items-start justify-between gap-3">
                <div className="min-w-0"><strong className="block truncate text-xs">{job.jobType}</strong><p className="mt-1 truncate text-[9px] text-muted">{job.scopeId ?? job.scopeType}</p></div>
                <Badge tone={jobTone(job.status)}>{job.status}</Badge>
              </header>
              <p className="mt-3 text-[9px] text-muted">{job.executionTarget}{job.leaseOwner ? ` · ${job.leaseOwner}` : ""} · 尝试 {job.attemptCount}</p>
              <time className="mt-2 block text-[9px] text-muted">{dateTime(job.leaseUntil ?? job.scheduledAt)}</time>
              {job.lastError && <p className="mt-3 rounded-xl bg-[#fce8ef] px-3 py-2 text-[9px] leading-5 text-[#943653]">{job.lastError}</p>}
            </article>
          ))}
        </div>
      </section>
