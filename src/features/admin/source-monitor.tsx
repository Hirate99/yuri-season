      ...
      {sources.map((source) => (
        <article className="border border-line bg-raised p-4.5" key={source.id}>
          <header className="flex justify-between">
            <span className={source.failureCount > 0 ? "text-[#9a324d]" : "text-[#55952b]"}>
              {source.failureCount > 0 ? <CircleX size={17} /> : <CircleCheck size={17} />}
            </span>
            <Badge>{cadenceLabel[source.cadenceProfile]}</Badge>
          </header>
          ...
          <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-line pt-3 sm:grid-cols-4">
            {[
              ["用途", purposeLabel[source.changeKind]],
              ["可信级别", source.trustLevel],
              ["上次检查", relativeTime(source.lastCheckedAt)],
              ["连续失败", source.failureCount],
            ].map(([label, value]) => (
              <div key={label}>
                <dt className="text-[8px] text-muted">{label}</dt>
                <dd className="mt-1 text-[9px] font-bold">{value}</dd>
              </div>
            ))}
          </dl>
          {source.lastError && (
            <small className="mt-3 block text-[9px] text-[#9a324d]">{source.lastError}</small>
          )}
        </article>
      ))}
    </div>
