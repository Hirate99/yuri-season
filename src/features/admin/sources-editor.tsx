...
function SourceForm({ item, accounts, busy, onSave }: {
  ...
  return (
    <form className="grid gap-3 pb-4 md:grid-cols-2" onSubmit={submit}>
      ...
      <AdminField label="公开文字"><select className={adminInput} name="publicTextMode" defaultValue={item?.publicTextMode ?? "full_with_translation"}><option value="full_with_translation">原文＋中文翻译</option><option value="full">完整原文</option><option value="excerpt">原文节选</option><option value="summary_only">仅中文摘要</option><option value="link_only">仅原链</option></select></AdminField>
