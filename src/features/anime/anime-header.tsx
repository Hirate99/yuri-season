...
export function AnimeHeader({ anime }: { anime: AnimeDetail }) {
  const cover = <CoverImage className="aspect-[3/4] w-full rounded-[6px] shadow-md ring-1 ring-black/[0.06]" src={anime.coverUrl} alt={`${anime.titleZh} 封面`} eager />;
  ...
  return (
    <header className="grid grid-cols-[100px_minmax(0,1fr)] gap-x-4 gap-y-5 py-2 sm:grid-cols-[180px_minmax(0,1fr)] md:gap-x-8 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-4">
      ...
      <div className="flex min-w-0 flex-col">
        ...
        <h1 className="mt-3 text-2xl leading-tight font-bold tracking-tight text-balance sm:text-3xl md:text-4xl">{anime.titleZh}</h1>
      ...
      <div className="col-span-2 min-w-0 sm:col-span-1 sm:col-start-2">
        ...
        <div className="surface mt-5 p-4 md:p-5">
          <dl className="grid grid-cols-2 gap-x-5 gap-y-4 text-sm md:grid-cols-4 [&_dt]:text-xs">
            <div className="min-w-0"><dt className="text-muted">制作</dt><dd className="mt-1 font-medium">{anime.studio ?? "—"}</dd></div>
            <div className="min-w-0"><dt className="text-muted">首播</dt><dd className="mt-1 font-medium">{shortDate(anime.premiereAt)}</dd></div>
            <div className="min-w-0"><dt className="text-muted">原作</dt><dd className="mt-1 font-medium">{anime.sourceMaterial ?? "—"}</dd></div>
            <div className="min-w-0"><dt className="text-muted">话数</dt><dd className="mt-1 font-medium">{anime.episodeCount ? `${anime.episodeCount} 话` : "—"}</dd></div>
          </dl>
