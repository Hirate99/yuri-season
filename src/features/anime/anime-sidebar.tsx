    ...
    <aside className="surface space-y-6 p-5 text-xs lg:sticky lg:top-24 lg:self-start">
      <section>
        ...
        <div className="mt-2 space-y-1">
          {anime.broadcasts.map((slot) => (
            <a className="block py-3 hover:text-accent" key={slot.id} href={slot.platformUrl ?? anime.officialUrl ?? "#"} target="_blank" rel="noreferrer">
        ...
        <section>
          ...
          <div className="mt-2 space-y-1">
            {verifiedEvents.map((event) => {
              ...
              return (
                <a className="grid grid-cols-[76px_1fr_auto] items-center gap-2 py-3 hover:text-accent" key={event.id} href={event.sourceUrl ?? "#"} target="_blank" rel="noreferrer">
      ...
      {anime.accounts.length > 0 && (
        <section>
          ...
          <div className="mt-2 space-y-1">
            {anime.accounts.map((account) => <a className="flex items-center justify-between py-3 hover:text-accent" key={account.id} href={account.url} target="_blank" rel="noreferrer"><span>{account.handle ?? account.platform}</span><ArrowUpRight size={12} /></a>)}
          </div>
        </section>
      )}
