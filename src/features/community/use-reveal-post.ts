...
export function useRevealPost(queryKey: QueryKey, query: { fetchNextPage: () => Promise<{ data?: Pages; isError: boolean }> }, setPage: (page: number) => void) {
  const client = useQueryClient();
  const [target, setTarget] = useState<string | null>(null);
  useEffect(() => {
    if (!target) return;
    const post = document.getElementById(`post-${target}`);
    if (post) { post.scrollIntoView({ block: "center" }); post.focus({ preventScroll: true }); }
    setTarget(null);
  }, [target]);
  ...
  return async (id: string) => {
    ...
    while (data) {
      ...
      if (page !== -1) { setPage(page); setTarget(id); return; }
