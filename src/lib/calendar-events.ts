...
export function eventIsOngoing(event: CalendarEvent, now = new Date()): boolean {
  if (!event.startsAt || event.eventType === "birthday" || event.status !== "scheduled")
    return false;
  ...
  return (
    event.endsAt !== null &&
    Date.parse(event.startsAt) <= now.valueOf() &&
    now.valueOf() < Date.parse(event.endsAt)
  );
}

export function prioritizeCalendarEvents(
    ...
    .map((event, index) => ({
      ...
      priority: eventOccursToday({ ...event, endsAt: null }, viewerTimeZone, now)
        ? event.eventType === "birthday"
          ? 3
          : 2
        : eventOccursToday(event, viewerTimeZone, now)
          ? 1
          : 0,
      duration:
        event.startsAt && event.endsAt
          ? Math.max(0, Date.parse(event.endsAt) - Date.parse(event.startsAt)) || 0
          : 0,
    }))
    .sort(
      (left, right) =>
        right.priority - left.priority ||
        (left.priority > 0 ? left.duration - right.duration : 0) ||
...
export function partitionCalendarEvents(events: CalendarEvent[], now = new Date()) {
  ...
  for (const event of events) {
    ...
    if (eventDay === null || endDay === null || endDay >= today)
      upcoming.push({ event, dateKey: eventDay });
  ...
  upcoming.sort((left, right) => {
    if (left.dateKey === null)
      return right.dateKey === null ? tieBreak(left.event, right.event) : 1;
