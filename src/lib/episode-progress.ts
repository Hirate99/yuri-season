...
export function resolveCurrentEpisode(input: EpisodeProgressInput, now = new Date()): number | null {
  if (input.latestVerifiedEpisode !== null) return cap(input.latestVerifiedEpisode, input.episodeCount);
