...
export function orderByNextBroadcast<T extends CatalogAnime>(anime: T[], now: Date): T[] {
  return anime
    .map((item) => ({
      anime: item,
      instant: item.primarySlot
        ? nextBroadcastInstant(item.primarySlot, now).valueOf()
        : Number.POSITIVE_INFINITY,
    }))
    .sort(
      (left, right) => left.instant - right.instant || left.anime.id.localeCompare(right.anime.id),
    )
    .map(({ anime: item }) => item);
}

export function orderByBroadcastFromToday<T extends CatalogAnime>(
  anime: T[],
...
export function partitionByAiringToday<T extends CatalogAnime>(
  ...
  for (const item of anime) {
    const instant =
      item.primarySlot && broadcastInstantOnViewerDate(item.primarySlot, viewerTimeZone, now);
