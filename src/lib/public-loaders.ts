...
export const loadHomeData = createIsomorphicFn()
  .server(async (input: PublicLoadContext & { seasonSlug?: string }) => {
    ...
    return serverData(
      service.home(input.serverContext?.viewerTimeZone ?? "Asia/Tokyo", input.seasonSlug),
    );
  })
  .client((input: PublicLoadContext & { seasonSlug?: string }) =>
    routeData(
      apiClient.api.home.$get({
        query: {
...
export const loadCalendarData = createIsomorphicFn()
  .server(async (input: PublicLoadContext & { seasonSlug?: string }) => {
    ...
    if (remote)
      return input.seasonSlug
        ? routeData(
            remote.api.seasons[":slug"].calendar.$get({ param: { slug: input.seasonSlug } }),
          )
  ...
  .client((input: PublicLoadContext & { seasonSlug?: string }) =>
    input.seasonSlug
      ? routeData(
          apiClient.api.seasons[":slug"].calendar.$get({ param: { slug: input.seasonSlug } }),
        )
...
export const loadFeedPage = createIsomorphicFn()
  .server(
    async (
      input: PublicLoadContext & { search: FeedSearch; cursor?: string; signal?: AbortSignal },
    ) => {
      const remote = productionApi(input);
      ...
      if (remote)
        return routeData(
          remote.api.feed.$get(
            { query: { ...feedQuery(input.search), cursor: input.cursor } },
            { init: { signal: input.signal } },
          ),
        );

      return (await publicService(input)).feed({
        limit: 20,
        query: input.search.q,
        animeSlug: input.search.anime,
        contentClasses: feedClasses(input.search),
        cursor: input.cursor,
      });
    },
  )
...
export const loadAnimeOptions = createIsomorphicFn()
  .server(async (input: PublicLoadContext) => {
    ...
    return remote
      ...
      : (await publicService(input)).catalog.options();
  })
...
export const loadAnimeRelatedData = createIsomorphicFn()
  .server(
    async (input: PublicLoadContext & { slug: string }): Promise<AnimeRelatedResponse | null> => {
      const remote = productionApi(input);

      if (remote)
        return routeData(remote.api.anime[":slug"].related.$get({ param: { slug: input.slug } }));
