...
export function catalogDescription(catalog: CatalogResponse, home = false): string {
  ...
  const titles = catalog.anime
    .slice(0, 2)
    .map((anime) => `《${anime.titleZh}》`)
    .join("、");
  ...
  return [
    ...
    home
      ? "还可追踪官方消息、声优与制作人员动态，并回到原始来源查看详情。"
      : "点击作品可查看简介、角色与声优、制作人员及相关情报。",
  ].join("");
}
...
export function publicationDescription({ item, document }: PublicationDetailResponse): string {
  ...
  const work =
    item.animeTitle && !summary.includes(item.animeTitle) ? `《${item.animeTitle}》：` : "";
...
export function seasonsDescription(data: SeasonsResponse): string {
  const labels = data.seasons
    .slice(0, 3)
    .map((season) => seasonName(season.label))
    .join("、");
