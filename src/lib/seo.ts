...
export const SITE_DESCRIPTION =
  "YuriSeason 为百合动画爱好者整理季度片单、每周放送表与最新情报。结合百合、关系向、女性群像和观察中标签选择想追的作品，查看角色与声优、制作人员、主题曲及官方动态，每条情报都可回到原始来源。";
...
export function publicationHead(data: PublicationDetailResponse | undefined, id: string) {
  return seoHead({
    ...
    image:
      data?.assets.find((asset) => asset.mimeType.startsWith("image/"))?.url ??
      data?.item.animeCoverUrl,
  });
}
