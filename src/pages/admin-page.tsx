...
export function AdminPage() {
  ...
  if (dashboard.error || !dashboard.data) return <div className="mx-auto max-w-3xl p-6"><EmptyState title="无法进入 Admin" detail={dashboard.error ?? "身份验证失败"} /><a className="mt-4 inline-flex rounded-full bg-white px-4 py-2 text-xs font-bold" href="/admin">重新验证</a></div>;
  ...
  return (
    <div className="mx-auto grid min-h-screen w-full max-w-[1540px] md:grid-cols-[190px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 px-4 pb-16 pt-6 sm:px-6 md:px-8 md:pt-9 xl:px-12">
        ...
        {actionError && <p className="mb-5 rounded-2xl bg-[#fce8ef] p-3 text-xs text-[#7d263f]">{actionError}</p>}
        ...
        {area === "works" && <WorksPanel anime={data.anime} seasons={data.seasons} busyId={busyId} onPatch={patchWork} onCreate={createWork} onChanged={dashboard.reload} />}
