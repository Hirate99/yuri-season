  ...
  return (
    <div className={page}>
      ...
      <section
        ...
        className="mt-6 flex items-center justify-between gap-4 rounded-lg bg-neutral-50 px-5 py-5"
      >
        <h2 id="community-heading" className="flex items-center gap-3 text-lg font-semibold">
          <MessagesSquare size={21} strokeWidth={1.5} className="text-accent" />
          作品讨论
        </h2>
        <Link
          to="/anime/$slug/discussions"
          params={{ slug: anime.slug }}
          className="inline-flex min-h-10 items-center gap-3 rounded-lg bg-neutral-800 px-4 text-xs font-semibold text-white transition hover:bg-neutral-700"
        >
          查看讨论
          <ArrowRight size={15} />
        </Link>
      ...
      <div className="mt-8 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="space-y-12">
          ...
          <Await promise={related} fallback={null}>
            {(content) => content && <DiscussionsSection discussions={content.discussions} />}
          </Await>
          <Await promise={related} fallback={null}>
            {(content) => content && <MediaSection media={content.media} />}
          </Await>
        </div>
