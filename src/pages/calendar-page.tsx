...
export function CalendarPage({
  ...
  const todayEvents = prioritizeCalendarEvents(
    data.events.filter((event) => eventOccursToday(event, calendarTimeZone, now)),
    calendarTimeZone,
    now,
  );
  ...
  const birthdayNames = todayEvents
    .filter((event) => event.eventType === "birthday")
    .map((event) => event.characterName ?? eventTitle(event));

  const eventGroups = partitionCalendarEvents(
    data.events.filter((event) => !todayIds.has(event.id)),
    now,
  );
  ...
  return (
    <div className={page}>
      ...
      <div>
        <nav
          className="sticky top-15 z-20 mb-4 grid grid-cols-7 gap-1 rounded-xl border border-line bg-white p-1 md:top-16 lg:hidden"
          aria-label="选择放送日"
        >
          {days.map((day) => (
            <button
          ...
          {days.map((day) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "min-w-0 lg:block lg:rounded-xl lg:border lg:p-2",
                  day !== selectedDay && "hidden",
                  day === today
                    ? "lg:border-accent/25 lg:bg-accent-soft"
                    : "lg:border-transparent lg:bg-raised",
                )}
              >
                <header className="mb-2 flex items-baseline justify-between gap-2 px-1 text-sm lg:py-1.5">
                  <strong className={day === today ? "text-accent" : ""}>
                    {weekdayLabel(day)}
                    {day === today && <span className="ml-1 text-xs font-normal">今天</span>}
                  </strong>
                ...
                <div className="grid gap-2">
                  {entries.map((entry) => (
                    ...
                    >
                      ...
                      <CoverImage
                        className="col-start-1 row-span-2 row-start-1 aspect-[3/4] w-9 rounded lg:float-left lg:mr-2 lg:mb-1 lg:w-7"
                        src={bangumiCoverUrl(entry.coverUrl, 100)}
                        alt={`${entry.titleZh} 封面`}
                      />
                      <strong className="col-start-2 row-start-2 min-w-0 text-[13px] leading-[18px] font-semibold group-hover:text-accent lg:block lg:text-xs">
                        {entry.titleZh}
                      </strong>
                      <span className="col-start-2 flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 lg:contents">
                        <span className="lg:mt-0.5 lg:block lg:[&>span]:text-[11px]">
                          <EpisodeProgressBadge episode={entry.currentEpisode} />
                        </span>
                        <small className="text-xs leading-4 text-muted lg:clear-both lg:block lg:pt-1.5 lg:text-[11px]">
                          {entry.slot.label}
                        </small>
                      </span>
                    </Link>
      ...
      <section className="mt-10">
        ...
        {todayEvents.length > 0 && (
          <section
            className="rounded-2xl border border-accent/15 bg-accent-soft/40 p-4 md:p-6"
            aria-labelledby="today-events-title"
          >
            <header className="mb-4 flex flex-wrap items-baseline justify-between gap-2">
              <h3 id="today-events-title" className="text-lg font-semibold">
                今天的事件{" "}
                <span className="ml-1 text-xs font-normal text-muted">{todayEvents.length}</span>
              </h3>
            ...
            {birthdayNames.length > 0 && (
              <div
                className="relative mb-4 overflow-hidden rounded-xl border border-rose-200/60 bg-gradient-to-r from-rose-50 via-white to-amber-50 px-5 py-5 md:px-6"
                role="note"
                aria-label="今日生日祝福"
              >
                <span
                  aria-hidden="true"
                  className="pointer-events-none absolute top-2 right-4 text-5xl text-rose-200/60"
                >
                  ✦
                </span>
                <p className="relative text-xs font-medium text-rose-700">今日生日</p>
                <p className="relative mt-2 text-lg leading-relaxed font-semibold text-rose-900 md:text-xl">
                  祝 {birthdayNames.join("、")} 生日快乐！
                </p>
              </div>
            )}
            <div className="grid gap-3 md:grid-cols-2">
              {todayEvents.map((event) => (
                <CalendarEventCard event={event} now={now} key={event.id} />
              ))}
            </div>
          </section>
        )}
