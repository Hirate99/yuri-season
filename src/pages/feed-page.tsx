    ...
    <div className={page}>
      <header className="page-header">
        ...
        >
          ...
          />
          <button
            className="grid size-10 shrink-0 place-items-center text-accent"
            type="submit"
            aria-label="搜索"
          >
            <Search size={18} />
      ...
      <section className="min-w-0" aria-label="情报列表" aria-busy={refreshing}>
          ...
          <div className="flex items-center justify-end gap-2">
            <div className="w-10 min-w-0 md:w-full">
              <AnimeCombobox
                ...
                anime={animeOptions}
                value={search.anime ?? ""}
                onChange={(anime) => onSearch({ ...search, anime: anime || undefined })}
              />
            </div>
        ...
        {search.anime && (
          <p className="mt-1 mb-2 text-xs text-accent">
            作品：
            {animeOptions.find((anime) => anime.slug === search.anime)?.titleZh ?? search.anime}
          </p>
        )}
...
function FeedResults({ search, refreshing }: { search: FeedSearch; refreshing: boolean }) {
  ...
  return (
    <>
      ...
      <VirtualWindowGrid
        ...
        estimateRowSize={190}
        hasMore={!refreshing && !feed.error && feed.hasNextPage}
      ...
      {feed.error && (
        <div className="mt-4 space-y-3">
          <EmptyState title="加载失败" detail={feed.error.message} />
          <button
            className={textButton}
            type="button"
            onClick={() => {
              if (feed.isFetchNextPageError) loadMore();
