...
export function PublicationPage({
  ...
  return (
    <div className="mx-auto w-[calc(100%-2.5rem)] max-w-[1120px] pt-6 pb-24 md:w-[calc(100%-3rem)] md:pt-8 md:pb-20">
      {onBack ? (
        <button
          className="inline-flex items-center gap-1.5 text-[11px] font-medium text-muted transition hover:text-ink"
          type="button"
          onClick={onBack}
        >
          <ArrowLeft size={13} />
      ...
      <article className="mt-8 md:mt-10">
        <header className="max-w-[820px]">
          <div className="flex flex-wrap items-center gap-x-2.5 gap-y-2 text-[11px] text-muted">
            <span className="rounded-full bg-[#f1efff] px-2.5 py-1 font-semibold text-[#5f53a7]">
              {contentLabel(item.contentClass)}
            </span>
            <span>
              {item.sourceName}
          ...
          <h1 className="mt-5 max-w-[780px] text-[clamp(1.75rem,2.8vw,2.5rem)] leading-[1.18] font-black tracking-[-0.035em] text-pretty">
            {item.title}
          </h1>
          <p className="mt-4 max-w-[760px] text-[15px] leading-6 text-[#555a62] md:text-base md:leading-8">
            {item.summary}
          </p>
        </header>
              ...
              <section aria-labelledby="source-text-heading">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <h2 id="source-text-heading" className="text-sm font-bold">
                      来源原文
                    </h2>
                  </div>
                  <span className="rounded-full bg-raised px-2.5 py-1 text-[10px] text-muted">
                    {textModeLabel[document.textMode]}
                    {document.sourceLanguage ? ` · ${document.sourceLanguage}` : ""}
                  </span>
                </div>
                <div className="mt-5 space-y-5 text-[15px] leading-8 text-[#30343a] md:text-base md:leading-9">
                  {paragraphs(document.publicText).map((paragraph, index) => (
                    <p key={index} className="wrap-anywhere whitespace-pre-wrap">
                      {paragraph}
                    </p>
                  ))}
                </div>
            ...
            {document?.publicTranslation && (
              <section className="mt-10" aria-labelledby="source-translation-heading">
                <div className="flex items-center gap-2.5">
                  ...
                  <h2 id="source-translation-heading" className="text-sm font-bold">
                    中文翻译
                  </h2>
                </div>
                <div className="mt-5 space-y-5 text-[15px] leading-8 text-[#454a52] md:text-base md:leading-9">
                  {paragraphs(document.publicTranslation).map((paragraph, index) => (
                    <p key={index} className="wrap-anywhere whitespace-pre-wrap">
                      {paragraph}
                    </p>
                  ))}
          ...
          <aside className="space-y-6 lg:sticky lg:top-24">
            <section className="rounded-2xl bg-raised p-5">
              ...
              <dl className="mt-4 grid gap-4 text-[11px] leading-5">
                <div>
                  <dt className="text-muted">发布者</dt>
                  <dd className="mt-0.5 font-semibold text-ink">
                    {document?.authorName ?? item.sourceName}
                  </dd>
                </div>
                {document?.sourceTitle && (
                  <div>
                    <dt className="text-muted">原始标题</dt>
                    <dd className="mt-0.5 text-ink">{document.sourceTitle}</dd>
                  </div>
                )}
                <div>
                ...
                {document && (
                  <div>
                    <dt className="text-muted">本站收录</dt>
                    <dd className="mt-0.5 text-ink">
                      <LocalDateTime value={document.capturedAt} />
                    </dd>
                  </div>
                )}
              </dl>
              <a
                className="group mt-5 flex w-full items-center justify-between rounded-xl bg-white px-4 py-3 text-xs font-semibold text-ink shadow-[0_5px_18px_rgba(15,23,42,0.06)] transition hover:shadow-[0_8px_24px_rgba(15,23,42,0.1)]"
                href={item.url}
                target="_blank"
                rel="noreferrer"
              >
            ...
            {(item.animeSlug || relatedAnime.length > 0) && (
              <section className="px-1">
                <h2 className="text-[10px] font-bold tracking-[0.14em] text-muted uppercase">
                  相关作品
                </h2>
                <div className="mt-3 flex flex-wrap gap-2">
                  {item.animeSlug && item.animeTitle && (
                    <Link
                      className="rounded-full bg-raised px-3 py-1.5 text-[11px] font-medium transition hover:bg-[#ece9fb]"
                      to="/anime/$slug"
                  ...
                  {relatedAnime
                    .filter((work) => work.slug !== item.animeSlug)
                    .map((work) => (
                      <Link
                        className="rounded-full bg-raised px-3 py-1.5 text-[11px] font-medium transition hover:bg-[#ece9fb]"
                        key={work.id}
