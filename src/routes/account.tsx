...
export const Route = createFileRoute("/account")({
  ...
  validateSearch: (search: Record<string, unknown>): { returnTo?: string } => ({
    returnTo:
      ...
      !/[\\\s]/.test(search.returnTo)
        ? search.returnTo
        : undefined,
  }),
  head: () => ({
    meta: [{ title: "我的账号 · YuriSeason" }, { name: "robots", content: "noindex" }],
  }),
  component: Account,
});

function Account() {
  // 会话在窗口重新聚焦时会刷新，期间保持登录表单挂载以保留验证码输入。
  const { data: session, isPending, error } = authClient.useSession();
  const { returnTo } = Route.useSearch();

  return (
    <CommunityFrame>
      <div className={session ? "" : "mx-auto max-w-sm py-6 md:py-12"}>
        <h1 className="mb-8 text-2xl font-bold">{session ? "我的账号" : "登录"}</h1>
        {session ? (
          <Profile user={session.user} />
        ) : (
          <>
            <FormError error={error} />
            <LoginForm returnTo={returnTo} checkingSession={isPending} />
          </>
        )}
      </div>
    </CommunityFrame>
...
function readPendingLogin(): PendingLogin | null {
  try {
    const value = pendingLoginSchema.parse(
      JSON.parse(sessionStorage.getItem(pendingLoginKey) || "null"),
    );
...
function LoginForm({ returnTo, checkingSession }: { returnTo?: string; checkingSession: boolean }) {
  ...
  const {
    ...
    resetField,
    formState: { errors },
  } = useForm({ defaultValues: { email: pending?.email || "", otp: "", name: "" } });
  ...
  return (
    <form
      noValidate
      onSubmit={handleSubmit((values) => action.mutate(values))}
      className="space-y-5"
    >
      <label className="block text-sm">
        ...
        <input
          ...
          {...register("email", {
            validate: (value) =>
              z.email().safeParse(value.trim()).success || "请输入有效的邮箱地址。",
          })}
        />
      </label>
      {sent && (
        <>
          <p className="text-sm text-muted">验证码已发送，5 分钟内有效。</p>
          <label className="block text-sm">
            6 位验证码
            <input
              className={field}
              ...
              maxLength={6}
              {...register("otp", {
                validate: (value) => !sent || /^\d{6}$/.test(value) || "请输入 6 位数字验证码。",
              })}
            />
          </label>
          <label className="block text-sm">
            昵称 <span className="text-muted">（新账号选填）</span>
            <input className={field} maxLength={32} autoComplete="nickname" {...register("name")} />
          </label>
        </>
      )}
      ...
      <Button
        type="submit"
        className="w-full"
        disabled={checkingSession || action.isPending || resend.isPending}
      >
        {action.isPending ? "请稍候…" : sent ? "登录" : "发送验证码"}
      </Button>
      {sent && (
        <div className="flex gap-4 text-xs">
          <button
            type="button"
            disabled={checkingSession || action.isPending || resend.isPending || retryIn > 0}
            onClick={() => {
              action.reset();
              resend.mutate();
            }}
          >
            {retryIn > 0 ? `${retryIn} 秒后重发` : "重新发送"}
          </button>
          <button
            type="button"
            disabled={action.isPending || resend.isPending}
            onClick={() => {
              storePendingLogin(null);
              setPending(null);
              resetField("otp");
          ...
          {resend.isSuccess && <span role="status">已重新发送</span>}
        </div>
      )}
    </form>
  );
...
function Profile({
  ...
  const save = useMutation({
    mutationFn: ({ name }: { name: string }) => authResult(authClient.updateUser({ name })),
    onSuccess: () => {
  ...
  const logout = useMutation({
    mutationFn: () => authResult(authClient.signOut()),
    onSuccess: () => {
      queryClient.clear();
    },
  });
  ...
  const activity = useQuery({
    ...
    queryFn: () => rpcData(apiClient.api.community.me.$get()),
    enabled: !user.banned,
  });
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        {user.banned && <p className="text-sm text-rose-700">账号已被禁言，暂时无法发帖或回复。</p>}
        <label className="block max-w-sm text-sm">
          ...
          <input
            ...
            required
            maxLength={32}
            {...register("name", { validate: (value) => Boolean(value.trim()) || "请填写昵称。" })}
          />
        </label>
        <FormError error={errors.name || save.error || logout.error} />
        <div className="flex gap-3">
          ...
          {save.isSuccess && (
            <span role="status" className="self-center text-sm text-green-700">
              已保存
            </span>
          )}
        </div>
      ...
      <section>
        <h2 className="mb-4 text-xl font-semibold">最近参与的讨论</h2>
        <FormError error={activity.error} />
        <div className="divide-y divide-line">
          {activity.data?.map((post) => (
            <div className="flex items-center justify-between gap-4 py-4 text-sm" key={post.id}>
              {post.threadHidden ? (
                <span className="text-muted">讨论已隐藏</span>
              ) : (
                <Link
                  to="/discussions/$id"
                  params={{ id: post.threadId }}
        ...
        {activity.data?.length === 0 && <p className="text-sm text-muted">暂无讨论记录。</p>}
        <p className="mt-4 text-xs text-muted">显示最近 50 条发言。</p>
      </section>
    </div>
  );
}
