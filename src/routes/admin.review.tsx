...
export const Route = createFileRoute("/admin/review")({
  validateSearch: (search: Record<string, unknown>): { view?: "inbox" | "published" } => ({
    view: search.view === "published" ? "published" : "inbox",
  }),
  component: Review,
});

function Review() {
  const { view = "inbox" } = Route.useSearch(),
    navigate = Route.useNavigate();

  const query = useQuery(reviewQuery),
    mutation = useMutation(decisionMutation);
  ...
  return (
    <>
      ...
      {mutation.error && (
        <p role="alert" className="mb-3 text-xs text-[#8b3048]">
      ...
      <AdminSegments
        ...
        items={[
          { id: "inbox", label: "待复核", count: summary.data?.counts.held },
          { id: "published", label: "已发布" },
        ]}
      />
      {query.data &&
        (view === "inbox" ? (
          <ReviewQueue candidates={query.data.heldCandidates} busyId={busyId} onDecision={decide} />
        ) : (
          <PublicationList
            publications={query.data.recentPublications}
            busyId={busyId}
            onWithdraw={(id, reason) => decide(id, "withdraw", reason)}
          />
        ))}
    </>
  );
