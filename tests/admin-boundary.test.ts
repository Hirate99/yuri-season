...
describe("Admin deployment boundary", () => {
  test("moves public Admin pages to the Access-protected Worker", () => {
    const response = redirectPublicAdmin(
      new Request(
        "https://yuri-season-radar.mskyurina.workers.dev/admin/resources?anime=kimishinu",
      ),
      publicEnv,
    );
    ...
    expect(response?.headers.get("location")).toBe(
      "https://i-yuri.com/admin/resources?anime=kimishinu",
    );
  });
  ...
  test("leaves the public site and token-authenticated Admin API on the public Worker", () => {
    ...
    expect(
      redirectPublicAdmin(new Request("https://example.com/api/admin/batches"), publicEnv),
    ).toBeNull();
    expect(
      redirectPublicAdmin(new Request("https://example.com/administrator"), publicEnv),
    ).toBeNull();
  });
  ...
  test("ignores an unsafe or malformed configured origin", () => {
    ...
    expect(
      redirectPublicAdmin(request, { DEPLOYMENT_ROLE: "public", ADMIN_ORIGIN: "not a URL" }),
    ).toBeNull();
    expect(
      redirectPublicAdmin(request, {
        DEPLOYMENT_ROLE: "public",
        ADMIN_ORIGIN: "http://example.com",
      }),
    ).toBeNull();
  });
});
