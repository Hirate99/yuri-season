...
describe("Admin related content", () => {
  test("atomically links a shared track, filling only missing credits", async () => {
    ...
    expect(
      database.sqlite
        .query("SELECT lyricist, composer, verified FROM music_tracks WHERE title = ?")
        .all(value.title),
    ...
    await expect(
      createAdminResource(database.binding(), "anime-kimishinu", conflicting),
    ).rejects.toMatchObject({ status: 409 });
    expect(
      database.sqlite.query("SELECT id FROM music_tracks WHERE title = 'Orphan track'").get(),
    ).toBeNull();
    ...
    await expect(
      createAdminResource(database.binding(), "anime-goodbye-lara", conflicting),
    ).rejects.toThrow("audit unavailable");
    await expect(
      createAdminResource(
        database.binding(),
        "anime-goodbye-lara",
        parseResourceWrite("theme_song", { ...value, arranger: "Uncommitted arranger" }),
      ),
    ).rejects.toThrow("audit unavailable");
    expect(database.sqlite.query("SELECT * FROM music_tracks").all()).toEqual(tracks);
    expect(await readAdminAnimeResources(database.binding(), "anime-goodbye-lara")).toEqual(
      resources,
    );
  });
  ...
  test("maintains events, media and concentrated discussion threads", async () => {
    ...
    const mediaWrite = parseResourceWrite("media", {
      ...
      originalUrl: "https://example.com/admin-media",
      previewUrl: null,
    ...
    const discussionWrite = parseResourceWrite("discussion", {
      platform: "百合会",
      title: "集中讨论",
      url: "https://example.com/admin-thread",
      note: null,
      isActive: true,
    ...
    expect(resources.events.find((item) => item.id === eventId)).toMatchObject({
      verified: true,
      eventType: "stream",
    });
    ...
    expect((await readMedia(database.binding(), animeId)).some((item) => item.id === mediaId)).toBe(
      false,
    );
    expect(
      (await readDiscussions(database.binding(), animeId)).some((item) => item.id === discussionId),
    ).toBe(true);
  ...
  test("reuses one discussion thread across multiple works", async () => {
    const write = parseResourceWrite("discussion", {
      platform: "百合会",
      title: "2026 夏季动画集中讨论",
      url: "https://bbs.example.test/thread-shared",
      note: null,
      isActive: true,
      lastActivityAt: "2026-08-11T12:00:00Z",
    });
    ...
    expect(
      (await readAdminAnimeResources(database.binding(), "anime-kimishinu")).discussions.find(
    ...
    expect(
      (await readDiscussions(database.binding(), "anime-taiari")).some(
        (item) => item.id === firstId,
      ),
    ).toBe(true);
    expect(
      database.sqlite.query("SELECT COUNT(*) AS count FROM discussions WHERE id = ?").get(firstId),
    ).toEqual({ count: 1 });
    await expect(
      deleteAdminResource(database.binding(), "anime-taiari", "discussion", firstId),
  ...
  test("shares verified music tracks while keeping per-work OP and ED usage", async () => {
    ...
    const ending = parseResourceWrite("theme_song", {
      ...opening.value,
      songKind: "ending",
      episodeRange: null,
    });
    ...
    const first = (
      await readAdminAnimeResources(database.binding(), "anime-kimishinu")
    ).themeSongs.find((item) => item.id === firstLink);
    const second = (
      await readAdminAnimeResources(database.binding(), "anime-taiari")
    ...
    const song = (await readAnimeDetail(database.binding(), "kimishinu"))!.themeSongs.find(
      ({ id }) => id === firstLink,
    )!;
    ...
    expect(
      (await readAdminAnimeResources(database.binding(), "anime-kimishinu")).themeSongs,
    ...
    expect(database.sqlite.query("SELECT COUNT(*) AS count FROM music_tracks").get()).toEqual({
      count: 1,
    });
  });
  ...
  test("requires a source for verified theme songs and jacket art", () => {
    ...
    expect(() =>
      parseResourceWrite("theme_song", {
        ...
        coverSourceUrl: null,
        sourceUrl: "https://example.com/music",
        verified: true,
        sortOrder: 0,
      }),
    ).toThrow("图片来源");
