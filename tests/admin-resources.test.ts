...
describe("Admin anime resources", () => {
  test("creates, updates and removes Japanese broadcast slots with one primary slot", async () => {
    ...
    const response = await api.request(
      `https://example.test/api/admin/anime/${animeId}/resources`,
      {
        method: "POST",
    ...
    const audit = database.sqlite
      .query("SELECT detail_json FROM audit_log WHERE entity_id = ?")
      .get(id) as { detail_json: string };
    expect(JSON.parse(audit.detail_json).principal).toEqual({
      kind: "automation",
      subject: "admin-token",
    });
    ...
    expect(
      (await readAdminAnimeResources(database.binding(), animeId)).broadcasts.some(
        (slot) => slot.id === id,
      ),
    ).toBe(false);
    expect(
  ...
  test("maintains staff, cast, verified accounts and low-frequency sources", async () => {
    const animeId = "anime-kimishinu";
    const staffId = await createAdminResource(
      database.binding(),
      animeId,
      parseResourceWrite("staff", {
        personId: null,
        name: "测试监督",
        nameNative: "テスト監督",
        primaryKind: "staff",
    ...
    const castId = await createAdminResource(
      database.binding(),
      animeId,
      parseResourceWrite("cast", {
        personId: null,
        characterName: "测试角色",
    ...
    const accountId = await createAdminResource(
      database.binding(),
      animeId,
      parseResourceWrite("account", {
        ownerType: "anime",
        ownerId: animeId,
        platform: "YouTube",
    ...
    expect(resources.cast.find((credit) => credit.id === castId)).toMatchObject({
      birthdayDay: 29,
      birthdayVerified: true,
    });
    expect(
    ...
    const birthday = database.sqlite
      .query(
        `SELECT id FROM events WHERE character_id =
      (SELECT character_id FROM cast_credits WHERE id = ?) AND event_type = 'birthday'`,
      )
      .get(castId) as { id: string };
    await expect(
      deleteAdminResource(database.binding(), animeId, "event", birthday.id),
    ).rejects.toMatchObject({ status: 404 });
    expect(
      database.sqlite
        .query("SELECT COUNT(*) AS count FROM audit_log WHERE entity_id = ?")
        .get(birthday.id),
    ).toEqual({ count: 0 });
    ...
    await expect(
      updateAdminResource(database.binding(), animeId, castId, castUpdate),
    ...
    expect(resources.accounts.find((account) => account.id === accountId)).toMatchObject({
      verified: true,
      monitorMode: "local",
    });
    ...
    expect(
      database.sqlite.query("SELECT account_id FROM research_sources WHERE id = ?").get(sourceId),
  ...
  test("refuses unverified birthday and account claims without evidence", () => {
    ...
    expect(() =>
      parseResourceWrite("cast", {
        ...
        characterProfile: null,
        personName: "声优",
        personNameNative: null,
        birthdayMonth: 4,
        birthdayDay: 31,
        birthdayYear: null,
        birthdayTimezone: "Asia/Tokyo",
        birthdaySourceUrl: "https://example.com",
        birthdayVerified: true,
        sortOrder: 0,
      }),
    ).toThrow("日期不成立");
