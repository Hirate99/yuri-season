...
describe("D1 read budgets", () => {
  test("keeps feed aggregates correct with multiple slots, withdrawn items and no visible items", async () => {
    ...
    const expected = database.sqlite
      .query(
        `SELECT COUNT(id) AS feedCount, MAX(published_at) AS latestFeedAt
      FROM feed_items WHERE anime_id = ? AND withdrawn_at IS NULL`,
      )
      .get(animeId) as { feedCount: number; latestFeedAt: string };
    expect(expected.feedCount).toBeGreaterThan(0);
    database.sqlite
      .query("UPDATE broadcast_slots SET is_primary = 1 WHERE anime_id = ?")
      .run(animeId);
    expect(await readAnimeSummaryBySlug(database.binding(), "kimishinu")).toMatchObject(expected);
    database.sqlite
      .query("UPDATE feed_items SET withdrawn_at = CURRENT_TIMESTAMP WHERE anime_id = ?")
      .run(animeId);
    expect(await readAnimeSummaryBySlug(database.binding(), "kimishinu")).toMatchObject({
      id: animeId,
      feedCount: 0,
      latestFeedAt: null,
    });
  ...
  test("shares credit fields while preserving public filtering and admin-only fields", async () => {
    ...
    expect(
      (await readAnimeDetail(database.binding(), "kimishinu"))!.cast.some(
        ({ characterId }) => characterId === "char-sheena",
      ),
    ).toBe(false);
    expect((await readAdminAnimeResources(database.binding(), detail.id)).cast).toContainEqual(
      expect.objectContaining({ characterId: "char-sheena", isMainGroup: false }),
    );
  });
  ...
  test("keeps catalog and calendar identifiers intact across batch mapping", async () => {
    ...
    expect(database.calls).toBe(2);
    expect(database.executedStatements).toBe(3);
    expect(calendar.entries).toContainEqual(
  ...
  test("batches admin resource sections and preserves character/person columns", async () => {
    ...
    expect(resources.discussions).toContainEqual(
      expect.objectContaining({
  ...
  test("checks staff, cast and unrelated people with one bounded query", async () => {
    const staff = database.sqlite
      .query("SELECT person_id FROM work_credits WHERE anime_id = ? LIMIT 1")
      .get("anime-kimishinu") as { person_id: string };
    for (const [personId, expected] of [
      [staff.person_id, true],
      ["person-takahashi-rie", true],
      ["missing-person", false],
    ] as const) {
      database.resetMetrics();
      expect(await personBelongsToAnime(database.binding(), "anime-kimishinu", personId)).toBe(
        expected,
      );
      expect(database.calls).toBe(1);
      expect(database.executedStatements).toBe(1);
