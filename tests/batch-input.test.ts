...
describe("research batch social-post text invariant", () => {
  ...
  test("accepts an auto-published social post with original text", () => {
    expect(researchBatchSchema.parse(socialBatch("publish", "Original post text."))).toMatchObject({
      observations: [{ publicText: "Original post text." }],
    });
  });
  ...
  test("allows a text-unavailable social post to remain held", () => {
    expect(researchBatchSchema.parse(socialBatch("hold"))).toMatchObject({
      observations: [{ candidates: [{ review: { decision: "hold" } }] }],
    });
  });
