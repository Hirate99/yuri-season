...
beforeEach(async () => {
  ...
  env = {
    ...
    BETTER_AUTH_SECRET: "community-test-secret-with-at-least-32-characters",
...
test("email OTP creates a verified account; codes are hashed, single-use, and logout revokes the session", async () => {
  ...
  expect((await request("auth/sign-in/email-otp", { email: user.email, otp })).status).not.toBe(
    200,
  );
  ...
  expect(
    JSON.stringify(db.sqlite.query("SELECT value FROM auth_verifications").all()),
  ).not.toContain(mail.at(-1)!.text.match(/\b\d{6}\b/)![0]);
...
function cooldown() {
  db.exec("UPDATE auth_users SET last_posted_at = 0");
}
...
test("OTP attempts and expiry cannot be bypassed, and missing mail configuration fails closed", async () => {
  ...
  for (let i = 0; i < 3; i++)
    expect((await request("auth/sign-in/email-otp", { email, otp: wrong })).status).not.toBe(200);
  ...
  expect(
    (await request("auth/sign-in/email-otp", { email: "expired@example.com", otp: expiredOtp }))
      .status,
  ).not.toBe(200);
  env.EMAIL = undefined;
  expect(
...
async function read<T>(path: string) {
  const response = await request(path, undefined, undefined, "GET");
  expect(response.status).toBe(200);
...
async function create(cookie: string) {
  ...
  const response = await request(
    `community/anime/${slug}/threads`,
    { title: "第 1 话讨论", body: "主帖", episode: 1, spoiler: true },
    cookie,
  );
...
test("floors, quotes and comments remain distinct; hidden floors preserve numbering and redact quotes", async () => {
  ...
  const comment = await request(
    `community/posts/${floorId}/comments`,
    { body: "楼内评论" },
    cookie,
  );
  ...
  expect(
    (await request(`community/posts/${commentId}/comments`, { body: "不允许嵌套楼内评论" }, cookie))
      .status,
  ).toBe(404);
  ...
  expect(
    (
      await request(
        `community/threads/${id}/replies`,
        { body: "主楼不能引用楼内评论", replyToId: commentId },
        cookie,
      )
    ).status,
  ).toBe(400);
  ...
  expect(
    (
      await request(
        `community/threads/${id}/replies`,
        { body: "第三楼", replyToId: floorId },
        cookie,
      )
    ).status,
  ).toBe(201);
  const floors = await read<Awaited<ReturnType<typeof listReplies>>>(
    `community/threads/${id}/replies`,
  );
  ...
  const comments = await read<Awaited<ReturnType<typeof listComments>>>(
    `community/posts/${floorId}/comments`,
  );
  ...
  expect(comments.items[1]).toMatchObject({
    replyToId: commentId,
    replyToName: "读者",
    replyToExcerpt: "楼内评论",
  });
  ...
  const hiddenComment = await read<Awaited<ReturnType<typeof listComments>>>(
    `community/posts/${floorId}/comments`,
  );
  ...
  expect(
    (
      await request(
        `community/posts/${floorId}/comments`,
        { body: "回复隐藏评论", replyToId: commentId },
        cookie,
      )
    ).status,
  ).toBe(400);
  expect((await admin(floorId, { kind: "post", hidden: true })).status).toBe(200);
  const hidden = await read<Awaited<ReturnType<typeof listReplies>>>(
    `community/threads/${id}/replies`,
  );
  ...
  expect(
    (await request(`community/posts/${floorId}/comments`, undefined, undefined, "GET")).status,
  ).toBe(404);
  const publicList = await read<Awaited<ReturnType<typeof listThreads>>>(
    `community/anime/${slug}/threads`,
  );
...
test("likes are idempotent, viewer-specific, reversible, and respect moderation", async () => {
  const reader = await login(),
    other = await login("liker@example.com");
  ...
  const like = (postId: string, liked = true, cookie?: string) =>
    request(`community/posts/${postId}/like`, { liked }, cookie, "PUT");
  const view = async (cookie?: string) =>
    (await request(`community/threads/${id}`, undefined, cookie, "GET")).json() as Promise<
      Awaited<ReturnType<typeof getThread>>
    >;
  const reaction = async (response: Response | Promise<Response>) =>
    (await response).json() as Promise<{ likes: number; liked: boolean }>;
  expect((await like(id)).status).toBe(401);
  for (let n = 0; n < 2; n++)
    expect(await reaction(like(id, true, reader.cookie))).toEqual({ likes: 1, liked: true });
  ...
  for (let n = 0; n < 2; n++)
    expect(await reaction(like(id, false, reader.cookie))).toEqual({ likes: 1, liked: false });
  ...
  expect(
    ((await comments.json()) as Awaited<ReturnType<typeof listComments>>).items[0],
  ).toMatchObject({ likes: 1, liked: true });
  expect((await view(other.cookie)).post.commentPreviews[0]).toMatchObject({
    likes: 1,
    liked: true,
  });
...
test("write boundaries enforce sessions, same origin, ownership, cooldown, locks, bans, and admin identity", async () => {
  ...
  cooldown();
  expect(
    (await request(`community/posts/${id}`, { body: "冒名编辑" }, other.cookie, "PATCH")).status,
  ).toBe(403);
  expect(
    (
      await request(
        `community/threads/${id}/replies`,
        { body: "跨串引用", replyToId: second.id },
        reader.cookie,
      )
    ).status,
  ).toBe(400);
  cooldown();
  expect(
    (await request(`community/threads/${id}/replies`, { body: "正常回复" }, reader.cookie)).status,
  ).toBe(201);
  expect(
    (await request(`community/posts/${id}/comments`, { body: "过快" }, reader.cookie)).status,
  ).toBe(429);
  await admin(id, { kind: "thread", locked: true });
  cooldown();
  expect(
    (await request(`community/threads/${id}/replies`, { body: "锁定" }, reader.cookie)).status,
  ).toBe(403);
  expect(
    (await request(`community/posts/${id}/comments`, { body: "锁定评论" }, reader.cookie)).status,
  ).toBe(403);
  expect(
    (await request(`community/posts/${id}`, { body: "锁定编辑" }, reader.cookie, "PATCH")).status,
  ).toBe(403);
  await admin(id, { kind: "thread", locked: false });
  await admin(reader.user.id, { kind: "user", banned: true });
  cooldown();
  expect(
    (await request(`community/threads/${id}/replies`, { body: "被禁言" }, reader.cookie)).status,
  ).toBe(403);
  expect((await request("admin/community", undefined, reader.cookie, "GET")).status).toBe(401);
  cooldown();
  expect(
    (await request(`community/posts/${id}/reports`, { reason: "垃圾内容" }, other.cookie)).status,
  ).toBe(200);
  const queue = await api.request(
    `${origin}/api/admin/community`,
  ...
  expect(
    (
      db.sqlite
        .query("SELECT count(*) AS count FROM audit_log WHERE action = 'moderate_community'")
        .get() as { count: number }
    ).count,
  ).toBe(4);
...
test("floor and comment pagination preserve hidden numbering and batch bounded previews", async () => {
  ...
  const insert = db.sqlite.prepare(
    "INSERT INTO community_posts(id, thread_id, author_id, body, floor, hidden, created_at, updated_at) VALUES (?, ?, ?, '内容', ?, ?, 1, 1)",
  );
  for (let floor = 2; floor <= 25; floor++)
    insert.run(`floor-${floor}`, id, user.id, floor, floor === 10 ? 1 : 0);
  const insertComment = db.sqlite.prepare(
    "INSERT INTO community_posts(id, thread_id, author_id, body, parent_post_id, hidden, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
  );
  for (const parent of [id, "floor-2", "floor-10"])
    for (let n = 1; n <= 12; n++)
      insertComment.run(
        `${parent}-comment-${n}`,
        id,
  ...
  db.resetMetrics();
  const first = await read<Awaited<ReturnType<typeof listReplies>>>(
    `community/threads/${id}/replies`,
  );
  ...
  const next = await read<Awaited<ReturnType<typeof listReplies>>>(
    `community/threads/${id}/replies?cursor=${encodeURIComponent(first.nextCursor!)}`,
  );
  expect([...first.items, ...next.items].map((post) => post.floor)).toEqual(
  ...
  const moreComments = await read<Awaited<ReturnType<typeof listComments>>>(
    `community/posts/${id}/comments?cursor=${encodeURIComponent(comments.nextCursor!)}`,
  );
