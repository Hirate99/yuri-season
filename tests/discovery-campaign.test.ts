...
describe("resumable discovery campaigns", () => {
  test("leases a bounded workset and recovers an expired lease", () => {
    ...
    expect(
      leaseCampaignQueries(campaign, 2, new Date("2026-08-11T20:00:00Z")).map((item) => item.id),
    ).toEqual(["q1", "q2"]);
    expect(
      leaseCampaignQueries(campaign, 2, new Date("2026-08-11T23:00:00Z")).map((item) => item.id),
    ).toEqual(["q1", "q2"]);
  ...
  test("prioritizes explicitly requested platforms without consuming unrelated work", () => {
    ...
    expect(
      leaseCampaignQueries(campaign, 500, new Date("2026-08-11T20:00:00Z"), (item) =>
        Boolean(item.platform && platforms.has(item.platform.toLowerCase())),
      ).map((item) => item.id),
    ).toEqual(["x", "instagram"]);
  ...
  test("derives memory and completes only recorded work", async () => {
    ...
    const results = [
      {
        queryId: "q1",
        searchedAt: "2026-08-11T20:15:00Z",
        status: "active" as const,
        outcome: "complete" as const,
        nextCheckAt: "2026-08-12T08:00:00Z",
        reasonCodes: ["quiet_but_unresolved"],
        notes: "no indexed result",
        hits: [],
      },
    ];
  ...
  test("refuses results that were not leased", async () => {
    ...
    await expect(
      memoryRecordsForResults(campaign, [
        {
          queryId: "q1",
          searchedAt: "2026-08-11T20:15:00Z",
          status: "active",
      ...
      memoryRecordsForResults(campaign, [
        {
          ...
          searchedAt: "2026-08-11T20:15:00Z",
          outcome: "complete",
          surface: "search_engine",
          status: "active",
          nextCheckAt: "2026-08-12T08:00:00Z",
          coverage: {
            reachedPreviousCursor: true,
            originalPostsInspected: 0,
          },
          hits: [],
        },
      ]),
  ...
  test("lets the agent schedule earlier while enforcing freshness and persists a stable cursor", async () => {
    ...
    const result = {
      ...
      searchedAt: "2026-08-11T20:15:00Z",
      outcome: "complete" as const,
      surface: "signed_in_timeline" as const,
      status: "active" as const,
      nextCheckAt: "2026-08-20T00:00:00Z",
      reasonCodes: ["recent_high_activity"],
      coverage: {
        reachedPreviousCursor: true,
        originalPostsInspected: 1,
        newestPostId: "110",
        newestPublishedAt: "2026-08-11T20:10:00Z",
        oldestPostId: "100",
      },
      hits: [
        {
          canonicalUrl: "https://x.com/work/status/110",
          title: "new",
          contentHash: null,
          outcome: "seen" as const,
          metadata: { platformObjectId: "110" },
        },
      ],
    };
    const records = await memoryRecordsForResults(campaign, [result]);
    expect(records[0].cursor).toMatchObject({
      committedPostId: "110",
      lastSurface: "signed_in_timeline",
    });
  ...
  test("persists verified discovery terms for later tag scans", async () => {
    ...
    const result = {
      ...
      searchedAt: "2026-08-11T20:15:00Z",
      outcome: "complete" as const,
      surface: "signed_in_search" as const,
      status: "active" as const,
      nextCheckAt: "2026-08-12T08:00:00Z",
      reasonCodes: ["new_term_found"],
      discoveredTerms: [
        {
          term: "#NewOfficialTag",
          kind: "official_tag" as const,
