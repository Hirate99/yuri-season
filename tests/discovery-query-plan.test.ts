  ...
  discussions: [
    {
      ...
      platform: "百合会",
      title: "专楼",
      url: "https://bbs.yamibo.com/thread-1.html",
      note: null,
      isActive: true,
      lastActivityAt: null,
      lastCheckedAt: null,
...
describe("discovery query planning", () => {
  test("keeps routine updates to registered verified X timelines", () => {
      ...
      accounts: [
        {
          ...
          ownerType: "anime",
          ownerId: anime.id,
          ownerLabel: anime.titleZh,
          platform: "X",
          handle: "@work",
          url: "https://x.com/work",
          verified: true,
          monitorMode: "local",
        ...
        },
        {
          ...
          platform: "X",
          handle: "@voice",
          url: "https://x.com/voice",
          verified: true,
          monitorMode: "local",
          verificationSourceUrl: "https://example.com/cast",
          verifiedAt: "2026-08-11T00:00:00Z",
        },
        {
          id: "account-author-x",
          ...
          verified: true,
          monitorMode: "local",
          verificationSourceUrl: "https://example.com/author",
          verifiedAt: "2026-08-11T00:00:00Z",
        },
        {
          id: "account-director-x",
          ownerType: "person",
          ownerId: "person-director",
          ownerLabel: "监督",
          platform: "X",
          handle: "@director",
          url: "https://x.com/director",
          verified: true,
          monitorMode: "local",
          verificationSourceUrl: "https://example.com/director",
          verifiedAt: "2026-08-11T00:00:00Z",
        },
        {
          id: "account-cast-instagram",
          ownerType: "person",
          ownerId: "person-1",
          ownerLabel: "声优",
          platform: "Instagram",
          handle: "voice",
          url: "https://www.instagram.com/voice/",
          verified: true,
          monitorMode: "local",
          verificationSourceUrl: "https://example.com/cast",
          verifiedAt: "2026-08-11T00:00:00Z",
    ...
    const queries = buildDiscoveryPlan({
      ...
      seasonLabel: "2026 夏",
      anime: [anime],
      resources: { "anime-1": monitoredResources },
      memory: [],
      memoryHits: [],
      now: new Date("2026-08-11T20:00:00Z"),
      force: false,
    ...
    expect(new Set(queries.map((query) => query.targetKey))).toEqual(
      new Set([
    ...
    expect(
      queries.every(
        (query) =>
          query.accountId &&
          query.platform?.toLowerCase() === "x" &&
          query.operation === "timeline_scan",
      ),
    ).toBe(true);
    ...
    expect(
      queries.some(
        (query) =>
          ...
          query.targetKey.startsWith("account:") ||
          query.targetKey.startsWith("community:") ||
          query.targetKey.startsWith("media:") ||
          query.targetKey.startsWith("music:"),
      ),
    ).toBe(false);
  });
  ...
  test("keeps general discovery from expanding into account discovery", () => {
    ...
    expect(
      queries.some(
        (query) => query.targetKey.startsWith("community:") || query.targetKey.startsWith("media:"),
      ),
    ).toBe(true);
  });
  ...
  test("plans database-aware community and account discovery without birthday polling", () => {
    const queries = plan();
    expect(queries.find((query) => query.targetKey === "music:theme-songs")).toMatchObject({
      searchKind: "official_news",
      priority: 5,
      maxFreshHours: 30 * 24,
    });
    expect(queries.filter((query) => query.targetKey === "community:yamibo-recent")).toHaveLength(
      1,
    );
    expect(queries.find((query) => query.targetKey === "community:yamibo-recent")).toMatchObject({
      scopeType: "season",
    ...
    expect(
      queries.find((query) => query.targetKey === "community:yamibo-recent")?.queryText,
    ).toContain("https://bbs.yamibo.com/forum-5-1.html");
    expect(queries.some((query) => query.targetKey === "community:tieba")).toBe(true);
    expect(queries.find((query) => query.targetKey === "community:moesen")).toMatchObject({
      priority: 3,
      maxFreshHours: 30 * 24,
    });
    expect(queries.some((query) => query.targetKey === "community:nga")).toBe(true);
    expect(
      queries.some((query) => query.scopeId === "character-1" && query.searchKind === "birthday"),
    ).toBe(false);
    expect(
      queries.some(
        (query) =>
          query.scopeId === "person-1" &&
          query.targetKey === "account:official-x" &&
          query.platform === "X",
      ),
    ).toBe(true);
    expect(
      queries.some(
        (query) =>
          query.scopeId === "person-1" &&
          query.targetKey === "account:official-instagram" &&
          query.platform === "Instagram",
      ),
    ).toBe(true);
    expect(
      queries
        .filter((query) => query.contentLane === "fanwork")
        .map((query) => query.platform)
        .sort(),
    ...
    expect(queries.find((query) => query.targetKey === "media:fanwork:instagram")).toMatchObject({
      priority: 1,
      maxFreshHours: 30 * 24,
    });
  });
  ...
  test("treats exhausted durable search memory as satisfied unless forced", () => {
    const exhausted = [
      {
        ...
        lastResultCount: 1,
        usefulResultCount: 0,
        searchedAt: "2026-08-10T00:00:00Z",
        nextSearchAt: null,
        notes: null,
        seenCount: 0,
        candidateCount: 0,
    ...
    expect(
      buildDiscoveryPlan({ ...common, force: false }).some(
        (query) => query.searchKind === "birthday",
      ),
    ).toBe(false);
    expect(
      buildDiscoveryPlan({ ...common, force: true }).some(
        (query) => query.searchKind === "birthday",
      ),
    ).toBe(true);
  });
  ...
  test("reschedules exhausted verified timelines when their next check is due", () => {
    ...
    const exhaustedTimeline = [
      {
        ...
        scopeId: "anime-1",
        searchKind: "social",
        targetKey: "updates:anime-1:account-work-x",
        queryText: "checked",
        status: "exhausted",
        lastResultHash: null,
        lastResultCount: 0,
        usefulResultCount: 0,
        searchedAt: "2026-08-10T00:00:00Z",
        nextSearchAt: "2026-08-11T00:00:00Z",
        notes: null,
        seenCount: 0,
        candidateCount: 0,
        publishedCount: 0,
    ...
    const queries = buildDiscoveryPlan({
      ...
      seasonLabel: "2026 夏",
      anime: [anime],
      resources: { "anime-1": timelineResources },
      memory: exhaustedTimeline,
      memoryHits: [],
      now: new Date("2026-08-11T20:00:00Z"),
      force: false,
      profile: "routine",
    ...
    expect(
      queries.some(
        (query) => query.accountId === "account-work-x" && query.operation === "timeline_scan",
      ),
    ).toBe(true);
  });
  ...
  test("keeps Mengzhan Bar separate from an ordinary work Tieba thread", () => {
    ...
    const queries = buildDiscoveryPlan({
      ...
      seasonLabel: "2026 夏",
      anime: [anime],
      resources: { "anime-1": tiebaResources },
      memory: [],
      memoryHits: [],
      now: new Date("2026-08-11T20:00:00Z"),
      force: false,
  ...
  test("limits birthday discovery to the recurring protagonist group", () => {
    ...
    const queries = buildDiscoveryPlan({
      ...
      seasonLabel: "2026 夏",
      anime: [anime],
      resources: { "anime-1": supportingResources },
      memory: [],
      memoryHits: [],
      now: new Date("2026-08-11T20:00:00Z"),
      force: false,
    ...
    expect(
      queries.some((query) => query.scopeId === "supporting-1" && query.searchKind === "birthday"),
    ).toBe(false);
    expect(
      queries.some((query) => query.scopeId === "person-1" && query.searchKind === "social"),
    ).toBe(false);
  });
  ...
  test("does not search ordinary staff accounts unless explicitly forced", () => {
    ...
    const common = {
      ...
      seasonLabel: "2026 夏",
      anime: [anime],
      resources: { "anime-1": resourcesWithOrdinaryStaff },
      memory: [],
      memoryHits: [],
      now: new Date("2026-08-11T20:00:00Z"),
      limit: 100,
    ...
    expect(
      buildDiscoveryPlan({ ...common, force: false }).some(
        (query) => query.scopeId === "person-staff" && query.searchKind === "social",
      ),
    ).toBe(false);
    expect(
      buildDiscoveryPlan({ ...common, force: true }).some(
  ...
  test("respects next search time unless a from-scratch audit is forced", () => {
    const futureMemory = [
      {
        ...
        usefulResultCount: 0,
        searchedAt: "2026-08-10T00:00:00Z",
        nextSearchAt: "2026-08-20T00:00:00Z",
        notes: null,
        seenCount: 0,
        candidateCount: 0,
        publishedCount: 0,
    ...
    expect(plan(futureMemory, true).some((query) => query.targetKey === "official:work")).toBe(
      true,
    );
  });
  ...
  test("does not rediscover registered official sources and work accounts", () => {
    ...
    const queries = buildDiscoveryPlan({
      ...
      seasonLabel: "2026 夏",
      anime: [anime],
      resources: { "anime-1": completeResources },
      memory: [],
      memoryHits: [],
      now: new Date("2026-08-11T20:00:00Z"),
      force: false,
    ...
    expect(
      queries.some(
        (query) =>
          query.targetKey === "updates:anime-1:account-work" &&
          query.queryText
            .toLowerCase()
            .includes("x official account timeline: https://x.com/work") &&
          query.queryText.includes("after:2026-07-12") &&
          query.priority === 5 &&
          query.maxFreshHours === 24,
      ),
    ).toBe(true);
    expect(
      queries.some((query) => query.scopeId === "person-1" && query.searchKind === "social"),
  ...
  test("turns existing unverified accounts into first-party verification work", () => {
    const resourcesWithPendingAccounts: AdminAnimeResources = {
      ...
      accounts: [
        {
          id: "account-work-pending",
          ownerType: "anime",
          ownerId: anime.id,
          ownerLabel: anime.titleZh,
          platform: "X",
          handle: "@work_pending",
          url: "https://x.com/work_pending",
          verified: false,
        ...
        {
          id: "account-cast-x-pending",
          ownerType: "person",
          ownerId: "person-1",
          ownerLabel: "声优",
          platform: "X",
          handle: "@voice_pending",
          url: "https://x.com/voice_pending",
          verified: false,
          monitorMode: "disabled",
          verificationSourceUrl: null,
          verifiedAt: null,
        ...
        {
          id: "account-cast-instagram-pending",
          ownerType: "person",
          ownerId: "person-1",
          ownerLabel: "声优",
          platform: "Instagram",
          handle: "voice_pending",
          url: "https://www.instagram.com/voice_pending/",
          verified: false,
          monitorMode: "disabled",
          verificationSourceUrl: null,
          verifiedAt: null,
    ...
    const common = {
      ...
      seasonLabel: "2026 夏",
      anime: [anime],
      resources: { "anime-1": resourcesWithPendingAccounts },
      memory: [],
      memoryHits: [],
      now: new Date("2026-08-11T20:00:00Z"),
      limit: 100,
    ...
    expect(
      ordinary.some(
        (query) => query.scopeId === "person-1" && query.targetKey.startsWith("account:official-"),
      ),
    ).toBe(false);
    expect(ordinary.filter((query) => query.targetKey.startsWith("verification:"))).toHaveLength(3);
    expect(
      ordinary.find((query) => query.targetKey === "verification:account-work-pending"),
    ).toMatchObject({ stage: "people", socialAuditEligible: true, contentLane: "official" });
    expect(
      ordinary.find((query) => query.targetKey === "verification:account-cast-x-pending")
        ?.queryText,
  ...
  test("plans projection reconciliation for verified songs missing Apple action or artwork", () => {
    ...
    const queries = buildDiscoveryPlan({
      ...
      seasonLabel: "2026 夏",
      anime: [anime],
      resources: { "anime-1": resourcesWithMusic },
      memory: [],
      memoryHits: [],
      now: new Date("2026-08-11T20:00:00Z"),
      force: false,
    ...
    const reconciliation = queries.find(
      (query) => query.targetKey === "music:theme-song-projection",
    );
  ...
  test("still reconciles a verified song with artwork but no Apple action", () => {
    ...
    const queries = buildDiscoveryPlan({
      ...
      seasonLabel: "2026 夏",
      anime: [anime],
      resources: { "anime-1": completeMusic },
      memory: [],
      memoryHits: [],
      now: new Date("2026-08-11T20:00:00Z"),
      force: false,
  ...
  test("plans work-related posts for verified cast accounts", () => {
    ...
    expect(
      queries.some(
        (query) =>
          query.targetKey === "updates:anime-1:account-cast" &&
          query.scopeType === "person" &&
          query.scopeId === "person-1" &&
          query.queryText.includes(
            "X verified cast/creator/staff timeline: https://x.com/voice_actor",
          ) &&
          query.queryText.includes('explicitly connected to "作品日本語"') &&
          query.operation === "timeline_scan" &&
          query.maxFreshHours === 24 &&
          query.priority === 3 &&
          query.accountId === "account-cast" &&
          query.personId === "person-1" &&
          query.contentLane === "cast" &&
          query.characterIds?.includes("character-1"),
      ),
    ).toBe(true);
    expect(
      queries.some(
        (query) => query.scopeId === "person-1" && query.targetKey === "account:official-x",
      ),
    ).toBe(false);
  });
  ...
  test("respects search-memory cooling for verified account updates", () => {
    ...
    const memory = [
      {
        id: "memory-account-update",
        scopeType: "anime",
        scopeId: anime.id,
        searchKind: "social",
        targetKey: "updates:anime-1:account-work",
        queryText: "old",
        status: "active",
        lastResultHash: null,
        lastResultCount: 0,
        usefulResultCount: 0,
        searchedAt: "2026-08-10T00:00:00Z",
        nextSearchAt: "2026-08-17T00:00:00Z",
        notes: null,
        seenCount: 0,
        candidateCount: 0,
        publishedCount: 0,
    ...
    expect(
      buildDiscoveryPlan({ ...input, force: false }).some(
        (query) => query.targetKey === "updates:anime-1:account-work",
      ),
    ).toBe(false);
    expect(
      buildDiscoveryPlan({ ...input, force: true }).some(
        (query) =>
          query.targetKey === "updates:anime-1:account-work" &&
          query.queryText.includes("X official account timeline: https://x.com/work") &&
          query.queryText.includes("after:2026-08-09"),
      ),
    ).toBe(true);
  });
  ...
  test("plans official X timeline and related-tag coverage for every current work", () => {
    ...
    expect(account).toMatchObject({
      priority: 5,
      contentLane: "official",
      operation: "timeline_scan",
      stage: "official",
      maxFreshHours: 24,
  ...
  test("reuses terms learned from recent official posts in the next tag scan", () => {
    ...
    const memory = [
      {
        ...
        scopeId: anime.id,
        searchKind: "social",
        targetKey: "updates:anime-1:x-tags",
        queryText: "old",
        status: "active",
        lastResultHash: null,
        lastResultCount: 1,
    ...
    expect(
      queries.find((query) => query.targetKey === "updates:anime-1:x-tags")?.queryText,
    ).toContain("#新企划标签");
  });
  ...
  test("keeps a social audit focused and orders whole-season coverage before people", () => {
    ...
    const queries = buildDiscoveryPlan({
      ...
      now: new Date("2026-08-11T20:00:00Z"),
      force: false,
      profile: "social-audit",
      limit: 100,
    });

    expect(queries.map((query) => query.stage)).toEqual(["official", "tags", "people"]);
    expect(
      queries.every((query) => query.searchKind === "social" && query.socialAuditEligible),
    ).toBe(true);
    expect(
      queries.some(
        (query) =>
  ...
  test("plans Instagram updates separately from X account coverage", () => {
    const monitoredResources: AdminAnimeResources = {
      ...
      accounts: [
        ...
        },
        {
          ...
          platform: "Instagram",
          handle: "@voice_actor",
          url: "https://www.instagram.com/voice_actor/",
          verified: true,
          monitorMode: "local",
          verificationSourceUrl: "https://example.com/official",
          verifiedAt: "2026-08-11T00:00:00Z",
    ...
    expect(
      queries.some(
        (query) =>
          query.accountId === "account-cast-instagram" &&
  ...
  test("monitors verified 2.5D project personas without requiring a title match", () => {
    const monitoredResources: AdminAnimeResources = {
      ...
      accounts: [
        {
          id: "account-persona-x",
          ownerType: "person",
          ownerId: "person-1",
          ownerLabel: "Project Persona",
          platform: "X",
          handle: "@project_persona",
          url: "https://x.com/project_persona",
          verified: true,
          monitorMode: "local",
          verificationSourceUrl: "https://example.com/project-member",
          verifiedAt: "2026-08-11T00:00:00Z",
        },
      ],
    };
    ...
    const monitoredResources: AdminAnimeResources = {
      ...
      accounts: [
        ...
        },
        {
          ...
          ownerLabel: "声优",
          platform: "Bluesky",
          handle: "voice.example",
          url: "https://bsky.app/profile/voice.example",
          verified: true,
          monitorMode: "api",
          verificationSourceUrl: "https://example.com/official",
          verifiedAt: "2026-08-11T00:00:00Z",
      ...
      sources: [
        {
          id: "source-account",
          accountId: "account-sourced",
          sourceType: "bluesky",
          changeKind: "feed_candidate",
          label: "声优 Bluesky",
          url: "https://public.api.bsky.app/xrpc/app.bsky.feed.getAuthorFeed?actor=voice.example",
          itemUrlTemplate: null,
