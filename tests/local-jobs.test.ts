...
describe("local discovery job leases", () => {
  test("leases, heartbeats and completes idempotently without storing the raw token", async () => {
    ...
    expect(await completeLocalJob(database.binding(), lease.id, input)).toMatchObject({
      status: "completed",
      duplicate: true,
    });
  ...
  test("recovers an expired lease and rejects the stale execution token", async () => {
    ...
    const expireBeforeWrite = spyOn(database, "batch").mockImplementationOnce((statements) => {
      database.sqlite
        .query("UPDATE update_jobs SET lease_until = datetime('now', '-1 minute') WHERE id = ?")
        .run(first.id);
    ...
    expect(
      database.sqlite.query("SELECT id FROM audit_log WHERE entity_id = ?").get(first.id),
    ).toBeNull();
    ...
    await expect(
      heartbeatLocalJob(database.binding(), first.id, first.leaseToken),
    ).rejects.toMatchObject({ status: 409 });
    await expect(
      heartbeatLocalJob(database.binding(), second.id, second.leaseToken),
  ...
  test("uses server-side retry policy for failed local work", async () => {
    ...
    expect(row).toMatchObject({
      status: "retry",
      last_error: "official site timed out",
