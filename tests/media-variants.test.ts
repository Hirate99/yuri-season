...
describe("media variants", () => {
  test("creates bounded WebP variants and import-ready metadata", async () => {
    ...
    expect(variants.map(({ variant, width, height, r2Key }) => ({ variant, width, height, r2Key })))
      .toEqual([
        {
          variant: "thumbnail",
          width: 320,
    ...
    expect(variants.every((variant) => variant.mimeType === "image/webp"
      && variant.byteSize > 0
  ...
  test("rejects unsafe R2 object paths", async () => {
    outputDirectory = await mkdtemp(join(tmpdir(), "yuri-media-variants-"));
    await expect(createMediaVariants("missing.jpg", "../private/image.jpg", outputDirectory))
