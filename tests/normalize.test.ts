...
describe("source normalization", () => {
  test("extracts same-page official news articles before unrelated links", async () => {
    const source = {
      ...
      changeKind: "feed_candidate" as const,
      label: "公式 NEWS",
      url: "https://example.com/news/",
      itemUrlTemplate: null,
      trustLevel: "official" as const,
      cadenceProfile: "local" as const,
      pollIntervalMin: 720,
      etag: null,
      lastModified: null,
      cursor: null,
  ...
  test("normalizes a community thread instead of action links", async () => {
    ...
    expect(items[0].metadata).toMatchObject({
      normalization: "community-thread",
      replyCount: 30,
      viewCount: 3492,
      lastPostId: 41576018,
    });
    ...
    const viewOnlyChanged = await normalizeSource(
      html.replace("3492", "4000"),
      "text/html",
      communitySource,
    );
    const replyChanged = await normalizeSource(
      html.replace('回复:</span> <span class="xi1">30', '回复:</span> <span class="xi1">31'),
      "text/html",
      communitySource,
    );
  ...
  test("maps numeric official news IDs to detail URLs", async () => {
    const source = {
      ...
      itemUrlTemplate: "https://example.com/news/{id}",
      trustLevel: "official" as const,
      cadenceProfile: "local" as const,
      pollIntervalMin: 720,
      etag: null,
    ...
    const items = await normalizeSource(
      JSON.stringify([{ uniqueId: 42, title: "活动决定" }]),
      "application/json",
      source,
    );
    expect(items[0].canonicalUrl).toBe("https://example.com/news/42");
  ...
  test("extracts a named official news collection and builds detail URLs from record IDs", async () => {
    const source = {
      ...
      changeKind: "feed_candidate" as const,
      label: "公式 NEWS",
      url: "https://example.com/api/site-data/init",
      itemUrlTemplate: "https://example.com/news/{id}/",
      trustLevel: "official" as const,
      cadenceProfile: "local" as const,
      pollIntervalMin: 720,
  ...
  test("ignores volatile Bangumi popularity metrics but keeps catalog changes", async () => {
    ...
    const popularityChanged = await normalizeSource(
      JSON.stringify({
        ...base,
    ...
    const catalogChanged = await normalizeSource(
      JSON.stringify({
        ...base,
