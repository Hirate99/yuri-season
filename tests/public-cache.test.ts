...
describe("public Redis cache", () => {
  test("falls back to the loader when Redis is not configured", async () => {
    ...
    expect(await readThroughPublicCache({}, "catalog", load)).toEqual({ value: 1 });
    expect(await readThroughPublicCache({}, "catalog", load)).toEqual({ value: 2 });
  });
  ...
  test("reuses a cached read and reloads it after version invalidation", async () => {
    ...
    expect(await readThroughPublicCache({}, "catalog", load, { client })).toEqual({ value: 1 });
    expect(await readThroughPublicCache({}, "catalog", load, { client })).toEqual({ value: 1 });
    await invalidatePublicCache({}, { client });
    expect(await readThroughPublicCache({}, "catalog", load, { client })).toEqual({ value: 2 });
  });

  test("keeps the D1 path healthy when Redis reads fail", async () => {
    const client = new MemoryCache();
    client.get = async () => { throw new Error("redis unavailable"); };
    await expect(readThroughPublicCache({}, "catalog", async () => ({ ok: true }), { client }))
