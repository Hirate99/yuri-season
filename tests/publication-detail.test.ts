...
async function publishedOfficialItem() {
  ...
  await applyCandidateDecision(database.binding(), candidateId, "publish", {
    reviewerType: "admin",
  });
  const publication = database.sqlite
    .query("SELECT id, media_id FROM feed_items WHERE candidate_id = ?")
...
describe("publication details", () => {
  ...
  test("keeps a Chinese translation separate from the original text", async () => {
    ...
    `,
      ...
      .run();
    await applyCandidateDecision(database.binding(), publication.candidateId, "publish", {
      reviewerType: "admin",
    });
  ...
  test("requests an existing smaller Bangumi cover for compact cards", () => {
    ...
    expect(bangumiCoverUrl(cover, 100)).toBe(
      "https://lain.bgm.tv/r/100/pic/cover/l/b3/26/541285_CYBZc.jpg",
    );
    expect(bangumiCoverUrl("https://example.com/cover.jpg", 100)).toBe(
