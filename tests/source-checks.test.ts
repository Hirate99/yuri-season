...
describe("local source health recording", () => {
  test.each([false, true])("shares success scheduling with Worker checks (urgent: %s)", async (urgent) => {
    database.sqlite.query(`UPDATE research_sources SET etag = 'old-tag', last_modified = 'old-date',
    ...
    const health = () => database.sqlite.query(`SELECT etag, last_modified, failure_count, last_error, lease_until,
      ROUND((julianday(next_check_at) - julianday(last_checked_at)) * 1440) AS delay
      FROM research_sources WHERE id = 'source-kimi-news'`).get();
    const expected = { etag: "old-tag", last_modified: "old-date", failure_count: 0,
      last_error: null, lease_until: null, delay: urgent ? 2 : 45 };
    expect(health()).toEqual(expected);
    expect(await recordSourceChecks(database.binding(), [{ sourceId: source!.id,
      checkedAt: "2998-01-01T00:00:00.000Z", outcome: "success", etag: null, lastModified: null,
    }])).toEqual({ received: 1, updated: 1 });
    expect(health()).toEqual(expected);
  });

  test.each([0, 71, 100])("shares capped failure backoff after %s failures", async (failures) => {
    database.sqlite.query("UPDATE research_sources SET failure_count = ?, lease_until = '2999-01-01' WHERE id = 'source-kimi-news'")
      .run(failures);
    const error = "x".repeat(900);
    await markSourceFailure(database.binding(), "source-kimi-news", new Error(error));
    const health = () => database.sqlite.query(`SELECT failure_count, last_error, lease_until,
      ROUND((julianday(next_check_at) - julianday(last_checked_at)) * 1440) AS delay
      FROM research_sources WHERE id = 'source-kimi-news'`).get();
    expect(health()).toEqual({ failure_count: failures + 1, last_error: error.slice(0, 800),
      lease_until: null, delay: Math.min(360, (failures + 1) * 5) });
    const check = { sourceId: "source-kimi-news", checkedAt: "2998-01-01T00:00:00.000Z",
      outcome: "failure" as const, error: "local failure" };
    await recordSourceChecks(database.binding(), [check]);
    expect(health()).toEqual({ failure_count: failures + 2, last_error: "local failure",
      lease_until: null, delay: Math.min(360, (failures + 2) * 5) });
    expect(await recordSourceChecks(database.binding(), [check])).toEqual({ received: 1, updated: 0 });
  });
  ...
  test("ignores local results older than a Worker check and skips empty batches", async () => {
    ...
    const before = database.sqlite.query("SELECT * FROM research_sources WHERE id = 'source-kimi-news'").get();
    expect(await recordSourceChecks(database.binding(), [{ sourceId: "source-kimi-news",
      checkedAt: "2000-01-01T00:00:00.000Z", outcome: "success", etag: "stale",
    }])).toEqual({ received: 1, updated: 0 });
    expect(database.sqlite.query("SELECT * FROM research_sources WHERE id = 'source-kimi-news'").get()).toEqual(before);
