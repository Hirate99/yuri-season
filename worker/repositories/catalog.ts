...
import { readAnimeSummariesForSeason } from "../db/read-models/anime";
import { readCalendarSlots, readEventsForSeason } from "../db/read-models/catalog";
import { animeTable, seasonsTable } from "../db/schema";
...
export async function currentSeason(db: D1Database): Promise<Season> {
  const [row] = await database(db).select(seasonProjection)
    ...
    .limit(1);
  if (!row) throw new HttpError(503, "灏氭湭閰嶇疆褰撳淇℃伅銆?");
  return row;
}
...
export async function seasonBySlug(db: D1Database, slug: string): Promise<Season> {
  const [row] = await database(db).select(seasonProjection)
    ...
    .limit(1);
  if (!row) throw new HttpError(404, "瀛ｅ害涓嶅瓨鍦ㄣ€?");
  return row;
}
