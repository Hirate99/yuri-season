...
export async function htmlArticleItems(raw: string, source: SourceRecord): Promise<NormalizedSource[]> {
  const document = parseDocument(raw, { decodeEntities: true });
  const articles = DomUtils.findAll((element) => element.name.toLowerCase() === "article", document);
  const entries = articles.map((article) => {
    const heading = DomUtils.findOne((element) => /^h[1-4]$/i.test(element.name), article.children);
    const time = DomUtils.findOne((element) => element.name.toLowerCase() === "time", article.children);
    ...
    if (!title || /^news$/i.test(title) || title.length < 4 || excerpt.length < title.length) return null;
  ...
  }).filter((entry): entry is NonNullable<typeof entry> => entry !== null).slice(0, 60);
