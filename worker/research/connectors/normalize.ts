...
function metaContent(document: Document, key: string): string | null {
  const element = DomUtils.findOne((candidate) =>
    candidate.name.toLowerCase() === "meta"
    && (candidate.attribs.property === key || candidate.attribs.name === key), document);
...
function canonicalFromHtml(document: Document, base: string): string {
  const element = DomUtils.findOne((candidate) =>
    ...
    && (candidate.attribs.rel ?? "").toLowerCase().split(/\s+/).includes("canonical"), document);
...
function communityTitle(document: Document, fallback: string): string {
  ...
  return value.replace(/\s+-\s+Powered by Discuz!\s*$/i, "")
...
function communityCount(document: Document, label: string): number | null {
  ...
  const labelIndex = spans.findIndex((element) => textFromNodes(element).replace(/\s+/g, "").startsWith(`${label}:`));
...
async function communityItem(raw: string, source: SourceRecord): Promise<NormalizedSource[]> {
  ...
  const lastPostId = DomUtils.findAll((element) => /^authorposton\d+$/i.test(element.attribs.id ?? ""), document)
...
function isActionLink(url: string): boolean {
  ...
  return target.pathname.endsWith("/home.php") || target.pathname.endsWith("/member.php") ||
    target.searchParams.has("formhash") || target.searchParams.get("mod") === "spacecp" ||
    ["favorite", "recommend", "report", "login", "logout"].some((action) => target.href.toLowerCase().includes(action));
}
...
async function htmlItems(raw: string, source: SourceRecord): Promise<NormalizedSource[]> {
  ...
  const document = parseDocument(raw, { decodeEntities: true });
  const links = DomUtils.findAll((element) => element.name.toLowerCase() === "a" && Boolean(element.attribs.href), document)
    .map((element) => ({
      url: resolveUrl(element.attribs.href, source.url),
      title: textFromNodes(element),
    }))
    .filter((item): item is { url: string; title: string } => Boolean(
      item.url && !isActionLink(item.url) && !/^news$/i.test(item.title) &&
      item.title.length >= 4 && item.title.length <= 180 &&
...
function xmlValue(entry: Element, names: string[]): string | null {
  ...
  const element = DomUtils.findOne((candidate) => lowered.has(candidate.name.toLowerCase()), entry.children);
...
async function feedItems(raw: string, source: SourceRecord): Promise<NormalizedSource[]> {
  ...
  const entries = DomUtils.findAll((element) => ["item", "entry"].includes(element.name.toLowerCase()), document).slice(0, 60);
  return Promise.all(entries.map(async (entry) => {
    const title = xmlValue(entry, ["title"]);
    const atomLink = DomUtils.findOne((element) => element.name.toLowerCase() === "link" && Boolean(element.attribs.href), entry.children);
    const link = resolveUrl(atomLink?.attribs.href ?? xmlValue(entry, ["link"]) ?? source.url, source.url) ?? source.url;
    ...
    : Array.isArray(root?.rows)
      ...
      : Array.isArray(root?.data)
        ...
        : Array.isArray(data?.rows)
          ? data.rows
          : [parsed];
  return Promise.all(values.slice(0, 100).map(async (value, index) => {
    const object: Record<string, unknown> = value && typeof value === "object" ? value as Record<string, unknown> : { value };
    const title = textFromHtml(String(object.name_cn ?? object.title ?? object.name ?? source.label));
    const sourceItemId = String(object.id ?? object.guid ?? object.uniqueId ?? object.url ?? index);
    const stableObject = source.source_type === "bangumi"
      ? Object.fromEntries([
        "id", "date", "platform", "images", "summary", "name", "name_cn", "infobox",
        "total_episodes", "eps", "meta_tags", "volumes", "series", "locked", "nsfw", "type",
      ].filter((key) => key in object).map((key) => [key, object[key]]))
      : Object.fromEntries(Object.entries(object).map(([key, field]) => [
        key,
        typeof field === "string" && ["body", "content", "description", "summary"].includes(key)
          ? textFromHtml(field)
          : field,
      ]));
    ...
    const uniqueId = typeof object.uniqueId === "string" || typeof object.uniqueId === "number"
      ? String(object.uniqueId)
      : null;
    const templatedId = uniqueId ?? (
      source.item_url_template && (typeof object.id === "string" || typeof object.id === "number")
        ? String(object.id)
        : null
    );
    const derivedUrl = templatedId
      ? source.item_url_template?.replaceAll("{id}", encodeURIComponent(templatedId)) ?? `${templatedId}.html`
      : null;
    const url = resolveUrl(directUrl ?? derivedUrl ?? source.url, source.url) ?? source.url;
    const rawPublishedAt = object.published_at ?? object.datetime ?? object.date ?? object.publishTime;
    const publishedAt = typeof rawPublishedAt === "number" || (typeof rawPublishedAt === "string" && /^\d{12,}$/.test(rawPublishedAt))
      ? new Date(Number(rawPublishedAt)).toISOString()
      : typeof rawPublishedAt === "string"
        ? rawPublishedAt
        : null;
    const previewUrl = typeof object.thumbnail === "string"
      ? resolveUrl(object.thumbnail, source.url)
      : typeof object.thumb === "string"
        ? resolveUrl(source.url.endsWith("/news/newslist.json") ? `thumbnail/${object.thumb}` : object.thumb, source.url)
        : null;
    return {
      sourceItemId,
      canonicalUrl: url,
...
function looksLikeFeed(raw: string): boolean {
  ...
  return Boolean(DomUtils.findOne((element) => ["rss", "feed"].includes(element.name.toLowerCase()), document));
}

export function normalizeSource(raw: string, contentType: string, source: SourceRecord) {
  if (source.source_type === "community") return communityItem(raw, source);
  if (contentType.includes("json")) return jsonItems(raw, source);
  if (contentType.includes("rss") || contentType.includes("atom") || looksLikeFeed(raw.slice(0, 2_000))) {
    return feedItems(raw, source);
  }
  return htmlItems(raw, source);
}

export function normalizerVersion(source: Pick<SourceRecord, "source_type">): string {
  return source.source_type === "community" ? "community-thread@2" : "generic@2";
}
